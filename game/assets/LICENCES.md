...
| `ui/boot_splash.png` | Boot splash image (phase 1.3), also the repo-root `Splash_h.jpg`/`Splash_v.jpg` source | **AI-generated** by the project owner using **Google Gemini** (paid account) | Project asset — see the note below |
| `ui/main_menu_panel.png` | Main menu panel frame (phase 1.1), copied from `assets/MainMenu.png` | **AI-generated** by the project owner using **Google Gemini** (paid account) | Project asset — see the note below |
...
|---|---|---|
| Kibyra UI packs | `assets/UI_Sprites/` (moved from repo-root `UI_Sprites/` 2026-08-08) | Free for personal and commercial use, but redistributing the original files is **not** permitted. Each developer downloads them — see [`UI_Sprites/README.md`](../../assets/UI_Sprites/README.md) |
| HUD icon source sheets | `assets/Icons/Icons_sheet_500x500.png`, `assets/Icons/MapIcons_500x100.png` (moved from repo-root `Icons/` 2026-08-08) | The sheets the individual 100×100 icons were cut from, plus `Icons/icons.txt` and `Icons/map_icons.txt` describing them. Working sources, like the root `UI_Design*.jpg` references — the cut icons under `game/assets/ui/icons/` are what ships. Kept so the 5×5 sheet's remaining empty slots can be filled from the same artwork |
...
| Control-group slot ring (PLAN.md 10.1) | `game/assets/ui/control_groups/group_slot_ring.png` | Copied locally from `assets/UI_Sprites/UI_dragon-huds/dragon-huds/rounded-bars/rounded-bar-transparent.png` (renamed on copy to avoid confusion with the actual rounded HP/mana/stamina bars in that same folder). Same Kibyra redistribution restriction as the row above; gitignored, each developer re-copies it from their own `assets/UI_Sprites/` download |
| Main menu buttons (PLAN.md 1.1) | `game/assets/ui/menu/{play,multiplayer,settings,credits,quit,resume,main_menu,back}_button.png`, `pause_icon.png` | Copied from `assets/UI_Sprites/UI_dragon-huds/dragon-huds/buttons/` (`playbutton.png` etc.) and `icons/pause-icon.png`, renamed to match their in-game role. Same Kibyra restriction; gitignored |
| Selection/HUD panel art (PLAN.md 8.1a/8.1b/8.1c/8.4) | `game/assets/ui/hud/{portrait_frame,health_bar,panel_background,toast_banner}.png` | Copied from `assets/UI_Sprites/UI_dragon-huds/dragon-huds/rounded-bars/avatar-frame.png`, `line-bars/line-bar-health.png`, `panels/panel-with-background.png`, `dialogue-box/dialogue-box-transparent.png`. Same Kibyra restriction; gitignored |
| Gemini working-source drops | `assets/*.png`, `assets/*.jpg` (e.g. `MainMenu.png`, `Age_and_Upgrade_bar.png`) | Same convention as the HUD icon source sheets above: the project owner drops raw generated art at the `assets/` root; whatever is actually used gets cut/copied into `game/assets/ui/` (e.g. `ui/main_menu_panel.png`) and only that copy ships. Not recursive -- a real subfolder like `UI_Sprites/` has its own row |
