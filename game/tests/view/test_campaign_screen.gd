...
## RESET PROGRESS WAS BUILT HERE AND MOVED, so there is nothing about it below. The owner's
## correction, 2026-09-06: *"the reset is per campaign, so on the progress tree page not the
## campaigns page."* Its tests went to `test_scenario_screen` with it, and so did the
## `progress_path` guard that keeps them off the developer's own save.
