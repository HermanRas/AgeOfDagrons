...
const AUDIO_MAP_PATH := "res://data/audio_map.json"
...
## audio_map.json, raw: which sound id each unit/building/resource def uses.
## Kept raw for the same reason as `_market` -- the accessor below IS the schema,
## and a class holding six optional StringNames would only be a second place for
## "absent means silence" to be got wrong.
...
func load_all(force := false) -> void:
	...
	_audio_map = _read_json(AUDIO_MAP_PATH)
	_resolved.clear()
...
## Declared sound IDs. This exists so a caller passing an ID that was never
## declared stays distinguishable from one that is declared and simply SILENT --
## the two cases look identical from the speaker and could not be more different
## in the code. An undeclared id is a caller bug and `AudioManager` says so; a
## declared id with no streams is a legitimate state, because the audio pack is
## optional (PLAN.md 3.2) and a build without it must play nothing rather than
## fail to boot. Same totality rule as `atlas_for()`, one layer up.
...
## The full seam entry for a sound: `streams`, `bus`, `gain_db`, the pitch range
## and `throttle_ms` (audio.json, generated by tools/stage_audio.py).
##
## Returns an EMPTY dictionary for an undeclared id rather than null, so a caller
## can read fields off the result without a null check and get the documented
## defaults from `get()`. Distinguishing declared-and-silent from undeclared is
## `has_sfx()`'s job, and `AudioManager` asks it first.
func sfx(sound_id: StringName) -> Dictionary:
	if not _loaded:
		load_all()
	return _audio.get(&"sfx", {}).get(String(sound_id), {})


func music(sound_id: StringName) -> Dictionary:
	if not _loaded:
		load_all()
	return _audio.get(&"music", {}).get(String(sound_id), {})


## Every declared sfx id, sorted. For the test suite and the licence audit, which
## both want to walk the vocabulary rather than guess at it.
func sfx_ids() -> Array[StringName]:
	if not _loaded:
		load_all()
	var ids: Array[StringName] = []
	for key in _audio.get(&"sfx", {}):
		ids.append(StringName(key))
	ids.sort()
	return ids


func music_ids() -> Array[StringName]:
	if not _loaded:
		load_all()
	var ids: Array[StringName] = []
	for key in _audio.get(&"music", {}):
		ids.append(StringName(key))
	ids.sort()
	return ids


## Which sound `def_id` makes for `event` -- `death`, `attack`, `trained`, `idle`,
## `complete`, `select`, `build`, `work`, or `voice` (audio_map.json).
##
## RETURNS `&""` FOR ANYTHING NOT MAPPED, and that is a state rather than an
## error: absence means silence. The monk has no `attack` because he carries no
## weapon, and a unit nobody has got round to mapping is quiet rather than
## borrowing a sword it is not holding. `AudioManager.play_sfx(&"")` is a no-op,
## so callers pass the result straight through without a guard.
##
## The three blocks are searched in order, which costs nothing (a def id is only
## ever in one of them) and means the caller does not have to know whether the
## thing it is holding is a unit, a building or a resource node -- `MatchAudio`
## reads snapshot entries, where that distinction is a matter of which fields
## happen to be present.
func entity_sfx(def_id: StringName, event: StringName) -> StringName:
	if not _loaded:
		load_all()
	for block in [&"units", &"buildings", &"resources"]:
		var entry: Dictionary = _audio_map.get(block, {}).get(String(def_id), {})
		if entry.has(String(event)):
			return StringName(entry[String(event)])
	return &""


## The order-acknowledgement voice line for `def_id`, e.g. `voice.female.select`.
## `&""` for anything with no `voice` in audio_map.json -- animals, ships and
## siege engines, which do not answer.
func entity_voice(def_id: StringName, line: StringName) -> StringName:
	var which := entity_sfx(def_id, &"voice")
	if which == &"":
		return &""
	return StringName("voice.%s.%s" % [which, line])


## Every def id that audio_map.json mentions. For the test that checks the map
## against the roster -- a sound mapped to a def that no longer exists is dead
## weight, and a def added without one is silent, and only walking both lists
## finds either.
func audio_mapped_ids() -> Array[StringName]:
	if not _loaded:
		load_all()
	var out: Array[StringName] = []
	for block in [&"units", &"buildings", &"resources"]:
		for key in _audio_map.get(block, {}):
			out.append(StringName(key))
	out.sort()
	return out


## Declared sound ids whose streams are not on disk -- i.e. the audio pack is
## absent or partial. The analogue of `missing_colour_atlases()`: silence is
## legitimate, so this is not a failure, but it must be *reportable* rather than
## something a player discovers by hearing nothing.
func silent_sfx_ids() -> Array[StringName]:
	var out: Array[StringName] = []
	for id in sfx_ids():
		var streams: Array = sfx(id).get("streams", [])
		var playable := false
		for path in streams:
