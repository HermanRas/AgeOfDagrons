...
| `vis.deer` | `idle`, `walk`, `die`, `decay` | 🟦 **BAKED (static only)** | Standing stag from `fauna/deer.xml`, 5 directions mirrored to 8, 128×128 canvas. Recipe: `tools/recipes/deer.toml`. **No animation** for the standing sprite itself (deliberately — 6.1a only needs a huntable food node, roaming is 6.1b): the actor's clips transfer by bone name (36/37, 97%) but the animation files and the mesh file describe the same skeleton ~31× apart, so unscaled location curves overshoot and the mesh tears into spikes — PLAN.md §13.2 item 8 has the measurements and the two dead ends recorded there. **The carcass gap this left is now closed, 2026-08-08** (requested in `asset_request.md` by the game-side agent): see `vis.deer_carcass` below — item 8's own suggested fix (scale the location curves by the measured ratio) is now a real isobake feature, not just a diagnosis. A.4 |
| `vis.deer_carcass` | `die` (posed, not played) | 🟦 **BAKED** | Fulfils the `asset_request.md` request for `res.deer`'s depleted-state visual. Same source (`fauna/deer.xml`), death clip's last frame (position 1.0, sampled via the `frames=2`/`loop=false` trick already used by `villager.toml`'s `decay`). **Fixes PLAN.md §13.2 item 8 rather than working around it:** isobake gained `anims.<name>.location_scale` (`recipe.py`, `zeroad.py`'s new `_scale_pose_location_curves`) to multiply a transferred clip's pose-bone *location* curves by a measured correction factor — exactly the fix item 8 named ("scale each action's location F-curves by that ratio — cheap once") but never implemented until now. Measured empirically for this clip at `0.0319` (close to item 8's own 3.70/116 ratio from the sibling walk clip); values above ~0.032 re-open a gap at the neck seam. A small tear at the neck/antler seam remains even at the best value found — it doesn't track with the scale value, so it reads as a pose/skinning-weight seam rather than a location-curve error, and is accepted as placeholder-quality (same "readability over anatomical accuracy" bar as `vis.fish`'s tuna substitution). 5 directions mirrored to 8 (mirroring flips which side it fell on — accepted, still reads as a dead deer), 192×192 canvas, clean bake, no clip warnings. Recipe: `tools/recipes/deer_carcass.toml`. **This unblocks any other quadruped's clip on the same import path** (boar/sheep/wolf/cavalry mounts) — each still needs its own measured `location_scale`, it is not a global constant |
...
| Attack/impact effects | ⚠️ **BLOCKED** | **Diagnosed 2026-08-08, not a recipe-writing task.** 0 A.D.'s combat VFX (`art/particles/*.xml` — `sparks.xml`, `destruction_dust_*`, `dust_cav.xml`, javelin/catapult flame trails, etc.) are all GPU particle emitters; there is no dedicated arrow-hit or sword-slash effect at all — hit reactions are pure `simulation/components/Attack.js` damage math, not per-hit visuals. isobake has zero particle or decal support: `zeroad.py` explicitly deletes stray light/camera objects on import but has no equivalent path for a particle emitter (the Pyrogenesis importer never produces one — mesh+armature+props only), and ground decals like the death-blood splatter (`props/units/blood_01.xml`) are explicitly skipped today (`zeroad.py:464-468`), not baked. No mesh-only alternative exists either — the closest thing, `props/units/weapons/tower_artillery_projectile_impact.xml`, is a hack that reuses a full humanoid trader mesh (`skeletal/celt_trader.dae`) as an invisible carrier purely to host particle props, not something baking would render usefully. A real fix needs a new isobake feature (particle simulation + billboard rendering, or decal support) — bigger in scope than the known variant-selection gap, since it's a different asset class, not a missing selector. Not attempted this pass |
...
| Cliffs | 🟦 **BAKED** | `vis.cliff`, `geology/stone_mediterranean_greek_furrowed_cliff.xml` — Mediterranean, civ-matching with grass1/dirta/sand/`terrain.rock`/`vis.stone_mine` (all Mediterranean picks). Single mesh, single variant, deterministic. 13.908 m tall, much taller than wide/deep (a vertical rock spire/face, not a squat outcrop like the stone mine) — needed two canvas jumps past the initial interpolated guess (896×704 clipped on 4 of 5 frames; 1536×1280 baked clean), consistent with [[isobake_canvas_sizing_by_interpolation]]'s wall-segment caveat about tall/elongated meshes overhanging the footprint-box estimate. **`ground_clip` rejected, unlike `vis.stone_mine`/`vis.tree_dead`:** the mesh's local origin sits well up its own height — z=0 cuts 13.908 m down to a bare 1.314 m sliver, which reads as buried-geometry-trim on other actors but here would throw away the entire visual subject; this actor is evidently authored to be partly sunk into a slope in 0 A.D.'s own terrain system, not freestanding, so the full un-clipped mesh was baked instead. 5 directions, 1536×1280 canvas, clean bake, no clip warnings. Recipe: `tools/recipes/terrain_cliff.toml` |
