...
All need foundation / construction / complete / damaged / rubble. **Complete state baked for 10 of 12** — mapped from `simulation/templates/structures/athen/*.xml`, which names the actor, footprint and rubble directly for every one of these, so none of it was guesswork. **Foundation and rubble for the first 9 are done as of 2026-08-08** — all 12 new size-specific recipes (7 foundations, 5 rubble, one size shared) baked with no canvas-edge clip warnings; see the per-row notes below.
