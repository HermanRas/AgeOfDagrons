...
def main() -> int:
    ...
    if short_units or warn_units:
        print(
            ...
            "\n"
            "  SEED      the actor declares several interchangeable variants in a\n"
            "            <group> and the eight colours rolled DIFFERENT ones. The tell\n"
            "            is that it reproduces EXACTLY -- same colours, same pixel\n"
            "            counts, on a re-bake or another machine -- so rebaking cannot\n"
            "            fix it at any width. The fix is `variant_seed` in the recipe,\n"
            "            pinned to the BASE recipe's id.\n"
            "\n"
            "FIXED AT SOURCE 2026-08-28, so a new instance of SEED means a colour recipe\n"
            "was hand-written or the generator regressed. isobake seeds the importer's\n"
            "RNG from the recipe id so a rebake reproduces itself (zeroad.py\n"
            "`_import_actor`) -- right for a base recipe, wrong for a colour variant,\n"
            "because every colour has a different id BY CONSTRUCTION and so rolled its\n"
            "own kit. `gen_player_colour_recipes.py` now emits\n"
            "`variant_seed = \"<base id>\"` into all 168.\n"
            "\n"
            "vis.fishing_ship was where it showed: celts/fishing_boat.xml has a six-\n"
            "variant group and three of them attach fish props, so blue and orange came\n"
            "out 0.84% short. It was blamed on the race for a fortnight, then on missing\n"
            "variant pinning; it was neither. Settled by baking ONE actor six times\n"
            "changing only the seed -- `.blue`/`.orange` gave 6498 opaque px and\n"
            "`.red`/`.white`/`.green` gave 6551, reproducing the staged split exactly.\n"
            "\n"
            "14 of the 21 colourable units have an ambiguous group (the archer has\n"
            "groups of 14 and 15 heads and helmets). The other 13 never reported SHORT\n"
            "because two helmets can have identical pixel counts -- so this check could\n"
            "not see them, and they were quietly showing different kit per player."
        )
