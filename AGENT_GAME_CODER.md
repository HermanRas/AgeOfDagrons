...
| ⚠️ **A CAMPAIGN INSTALLED INTO `user://content/scenarios/` SHADOWS THE REPO'S COPY, AND THE SUITE SAYS SO** | `test_campaign_screen::test_the_shipped_campaign_loads_without_complaint` fails with *"campaign 'HowToPlay' in `user://content/scenarios/` is shadowed by the copy in …/scenarios"*. **It is not a code failure and it is not noise** — it is 0.3's content-install path having put a pack copy on the machine, after which **the game loads that instead of the repo's content and edits to `scenarios/` appear to do nothing.** Met on 2026-09-11 when a playtest installed one mid-session. The fix is to delete `%APPDATA%\Godot\app_userdata\AgeOfDragons\content\scenarios\<campaign>` — **but it is the owner's data, so ask rather than tidying it away.** |
