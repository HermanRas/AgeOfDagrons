...
Both projects. **Game 2502/2503** and **MapMaker 377/377**, up 123.

⛔ **THE ONE FAILURE WAS NOT "THE OWNER'S DATA", AND SAYING SO HERE IS WHAT KEPT IT ALIVE.** This
section recorded it as §6's `user://content/scenarios/` shadow — an installed pack, environmental,
not a code fault. The owner asked *"what is making it fail and how do we fix it?"* on 2026-09-12
and the answer was three defects stacked: every directory in this Google-Drive repo is Windows
read-only, so a recursive delete removed a pack's files and could not remove its folders; the husk
that left behind had no `campaign.json` in it at all; and `Campaigns.discover()` called it a
shadowing campaign **without ever asking whether it was a campaign**. Fixed, and the game suite is
**2508/2508 — green for the first time**. See §6's three new rows.
