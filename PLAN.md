...
Primary source is the project website `https://aod.dragoon.co.za/` (website source in
`web/server/app/`, which **mirrors the server's `/opt/aod/` exactly** since 2026-09-04, so a
file's path under `app/` is its URL path — see `web/README.md`):
...
✅ **THE PRICE OF THE MOVE IS PAID: 0.3 LANDED 2026-09-03 AND A CAMPAIGN HAS NOW REACHED A PHONE.**
This paragraph said, correctly for two days, that *a campaign cannot be tested on the phone until
0.3 lands* — `user://` on Android is internal app storage and is not `adb push`-able, and the
override is editor-only, so the delivery path to a device was `AssetPacks` and nothing else. It is
built, and the `howtoplay` pack installs and loads on hardware. **The SHAPE of the dependency is
still what a reader needs, which is why this is rewritten rather than deleted:** the dev override
is editor-only, a device reads `user://content/`, and something has to put content there. Two
escapes were listed here and neither was needed; the one worth remembering is that a **debug**
build never had to wait at all, because `run-as` reaches the app's own files directory
(`Docs/MOBILE_DEPLOY.md`).
...
AOD_Mobile/
  ...
  web/server/                 # WEBSITE + HOST CONFIG, laid out as a MIRROR of the box's
                              # /opt/aod/ (reorganised 2026-09-04). `app/` is the document
                              # root, so a deploy is a recursive copy with no path
                              # translation; `build_packs.py` writes into app/downloads/
  ...
  tools/                      # OFFLINE pipeline - Python, never shipped
    ...
    build_packs.py            # 0.3, built 2026-09-03: campaign/map zips + manifest +
                              # checksums. The ART `.pck` half is NOT written yet
    packs.source.json         # what build_packs.py publishes, and at what version
...
| `NestSystem` | The dragon claim — hatch, count 3600 ticks, hand over, deny (§13.2) | ✅ |
| `AISystem` | Drive AI players (emits Commands like any player) | ✅ 12.2a |

⚠️ **`TechSystem` IS NOT ON THIS LIST AND MUST NOT BE ADDED.** §9.3 promised one for *"research
timers, stat modifiers"* and neither half wanted a system. The timers are the **production
queue's** — a research is an entry on it and `ProductionSystem` has counted those down since 5.4,
so a second system ticking the same counter would be two owners of it. The modifiers are a pure
function of which techs a player holds, so `TechMods` is a **static resolver** in the shape of
`Formation`, `WallPlan` and `Diplomacy`.
...
are the exception: **persisted in `SimPlayer`** via `SetControlGroupCommand` so they survive the
match being saved and reloaded. ⚠️ **This row said "so they survive reconnect" until 2026-09-21**,
when reconnect went out of scope (§12.1b) — **the placement is still right and the reason
changed**: `SaveGame` captures and restores `control_groups`, so groups kept in the view would
reload empty. Worth more than the correction: when a ruling removes a justification, check
whether the code it justified has another one before touching the code.
...
4. **Three sliders, not six.** UI, VOICE and AMBIENT route into SFX, so Master/Music/Effects covers
   the whole mix. This is the answer to what §13.2's audio row left open before it closed.
...
Game.tscn         [MVP]  the match - game_scene.gd
  ...
  CanvasLayer (HUD, above): ControlGroupsHud (top-left), age header + pause +
                         ...
                         ResultScreen, ScenarioBriefing (15.6, modal),
                         ObjectiveTracker (15.6, top-left beside the ring stack)
Credits.tscn      [MVP]
...
Campaign.tscn            15.4 - a three-line shell; CampaignScreen builds itself in _init()
Scenario.tscn            15.5 - the same shape, for the same reason: the list is runtime data
DownloadScreen.tscn      3.2 / 0.3 - BUILT 2026-09-03, plus the DOWNLOAD MORE browser
...
| `building.castle` | 3 | **+40 pop.** Knight; elite swordsman + trebuchet from 4. ~~the dragon~~ — dropped 2026-09-04, see `unit.dragon`'s row | `iber/fortress` · `rome/fortress` |
...
| `building.dragon_nest` | ✅ 2026-09-04 | **Not buildable** (`buildable: false`) — a map POI (13.2), placed by `MapGenerator._place_nest` since 13.2a. 10×10, `blocks_movement: false`: the footprint stops a house being dropped on it, and you walk through a henge. **`leaves_rubble: false`** (13.2b) — there is no rubble bake and a razed henge is an absence, not a pile; left as rubble, a rival who had just denied a dragon claim would watch an apparently untouched nest for a minute. **hp 1200 is provisional and wants the owner's eye** — it is a number about how defensible the 360 s window is, not about how sturdy stones are | 22× `prop_nest_bush`, 12× `prop_standing_stone`, `prop_shrine_celtic` — **all three baked, staged AND now declared**. ⚠️ **The shrine is the CORE**, not a prop: `EntityView` draws a visual before its props, so "no core" would have painted a magenta placeholder inside its own decoration. The bush is `flora/trees/temperate_bush_biome.xml`, not the `bush_badlands` this row used to name |
...
| `unit.dragon_baby` | — | ✅ 2026-09-06. Spawned at the nest by `NestSystem` when the mother dies; 360 s later it is REPLACED by a `unit.dragon` owned by whoever killed her | `vis.dragon_baby` = `vis.dragon_rigged` at **`"scale": 0.2`** (owner, 2026-09-04; this said 10%). **No bake of its own, and there must never be one** — a 20% copy of the largest atlas in the game is 7.7 MB of identical frames. **Also what `Mode.TROPHY` needs** (11.2), for a different job — see 13.2. ✅ **AND SINCE 2026-09-07 IT DOES BOTH JOBS FOR REAL**: `is_trophy: true`, `can_garrison: false` (4.8c), and **`speed` 0 → 55** so the owner can move a trophy about. ⚠️ **The two jobs are told apart by `owner_id` and nothing else** — gaia's is the claim's, a player's is a trophy — which is load-bearing in `NestSystem` (see 11.2) and is why the speed is safe: 13.2's hatchling still cannot move, because no player can order a gaia unit and its roam radius is 0 |
...
The asset seam, the placeholder renderer, the licence audit, the sim skeleton, `state_hash()` /
`Replay` / the `sim/` boundary check, the working roots, and the `isobake` render pipeline. What
is worth carrying forward is not that they were built but the rules they fixed:

**`AssetPacks` (0.3) — the decisions, not the delivery.**

- **Two content classes**, the owner's split: `required` is fetched at boot with nobody asked,
  optional is browse-and-pick.
- **Two verbs, deliberately.** Art and audio are **mounted**; a campaign is **installed** into
  `user://content/`, because content that cannot be updated, removed or shared is not the feature
  that was asked for.
- **The offline boot must reach the front door** rather than stall on the manifest (§3.2's
  placeholder rule). `MANIFEST_TIMEOUT_SECONDS` bounds the small request only — there is
  deliberately **no timeout on a payload**, so a slow link is not mistaken for a broken server.
- **The pack version is the packer's own content digest.** `attribution.actor` never moves on a
  rebake and `generator.isobake_commit` is absent from 67 atlases, so neither can serve. The
  packer reads `visuals.json` and **never the directory**, so it packs what the seam can ask for
  and unreferenced bakes drop out by construction. ⚠️ **Any rebake that moves a staged byte moves
  the digest, and the build refuses to publish until the version is bumped by hand.**
- ⛔ **A DROPPED CONNECTION DELETES ITS OWN PARTIAL FILE, and `set_download_file()` truncates
  rather than appends.** Both premises the original design argued from were the opposite of true.
  This is why a pack arrives in bounded 4 MiB chunks appended by hand to `<id>.part`: whatever is
  in flight when a connection dies is lost regardless, so a single-request resume would restart
  from zero on the first attempt — which is the failure being fixed. The chunk size is *how much
  one drop is allowed to cost*. ⚠️ **A `.part` is CLAIMED** by a sidecar recording sha256, size
  and version, because resuming into the wrong bytes is silent and throws away hours.
- ⛔ **A mounted pack cannot be un-mounted** — Godot has no `unload_resource_pack()` — so DELETE
  is offered on content and not on art. Permanent, not a defect; it bounds what an art-pack UI
  can promise about retrying.

**Ownership:** `tools/build_packs.py` + `packs.source.json` are the game-code agent's by the
owner's decision, a fourth named exception inside `tools/`. Flagged in `AGENT_GAME_CODER.md` §1 as
the exception that does not justify itself — it will eventually read the art pipeline's output, so
the ownership gets raised again rather than quietly drifting.

### Phase 1 — Main menu

*The menu, boot card, credits, settings, skirmish screen and HOW TO PLAY are all built. The
decisions behind them that are still live:*

- ⛔ **A `TextureRect` DEFAULTS TO `EXPAND_KEEP_SIZE`, WHICH MAKES THE TEXTURE'S OWN SIZE THE
  CONTROL'S MINIMUM — and a minimum size beats anchors.** `PRESET_FULL_RECT` therefore gave a
  1376×768 control pinned to the top-left of a 1152×648 viewport, and the boot plate hung off two
  edges for weeks while the stretch mode was blamed. `expand_mode = EXPAND_IGNORE_SIZE` is the
  fix. `preview_menus` photographs it and derives the painted region, because **no node exposes
  it** — this is not assertable any other way.
- **`Credits.tscn` hardcodes its text** because CREDITS.md lives outside `res://`.
- **Settings is an overlay, not a scene.** It holds one thing (volume, 8.2b) and a whole screen to
  get back from is a poor trade for three sliders. `Settings.tscn` in §8 stays unbuilt and this is
  why. Built in code rather than authored, so the editor cannot silently reformat it.
- **HOW TO PLAY is a pager, not a scroll**, and its pages carry their own instructions painted onto
  real captures of this HUD. So the screen writes no tutorial prose that can drift: when a control
  moves, the fix is a new capture. Six wide screenshots in a `ScrollContainer` give each one a
  sixth of the height it was drawn for on a handset. `test_help_screen` asserts every page's file
  is actually staged — a `PAGES` table of strings stays true when the images are missing.

#### 1.7 Lobby and multiplayer UI — the design references, and what they show that does not exist

Three artefacts, **ahead of the systems rather than behind them**, which is what makes them worth
keeping.

**`UI_Design_Lobby.png`** — the hosting lobby. Still missing: a **Faction** column (one
civilisation is a locked v1 decision, §1 — 9.5 territory), a **Game Type / Victory Conditions**
picker offering **Regicide** (declared and inert, 11.2), a **Map Size** choice (size is currently
derived from the player count, §11.2's area rule), and **in-lobby chat** (8.6 is a wireframe with
no transport). Confirmed already right: eight slots, per-slot AI difficulty, the map preview, the
four minimap corner buttons, and the team column.

⛔ **THE DIPLOMACY SCOPE IS THE OWNER'S AND IS NOT AN UNFINISHED EDGE.** Shared vision is in — an
ally's eyes are your eyes, in the grid and in the snapshot both (§11.4a), and the owner reserved
the keep-or-drop decision on it pending play. **Shared control, ally repair/garrison/healing and an
ally-aware AI are OUT**: *"we will not be interacting with any other player units or buildings, no
garrison, no repair, no healing… happy with AI attacking opposing teams but not team members."* So
an ally's entity taps to SELECT and to nothing else, and every owner-gated command stays
owner-gated. ⚠️ **A slot opens on `–` because four team numbers cannot give eight slots a side
each**, so a free-for-all is what the game plays unless somebody deliberately makes a team.

**`UI_Design_Hosting.png`** — a **server browser**: filters by map and game type, a game-**version**
filter, named servers with player counts and Playing/Lobby status. **12.1b LAN discovery is the
subset of this that is planned**; the rest implies a master server, server naming and version
negotiation, none of which is scoped. ⚠️ **Read it before designing 12.1b**, so LAN discovery does
not paint itself into a corner this cannot grow out of. A disabled wireframe exists
(`ServerBrowserPanel`) and its header lists the four things a real one needs — the one genuinely
missing from the systems is a host **NAME**: `MatchConfig` carries everything else the columns
show, and a list of four IP addresses is a list nobody can choose from.

**`web/server/app/player-colour-ladder.html`** — the research behind the eight-colour palette: CIE
`L*` spread so four of the eight separate by **lightness** rather than hue, dichromacy safety per
pair, and the argument that A.6's tint **must not be a multiply** (against white it is a no-op and
the player looks untinted). Cross-referenced from §9's palette and §12A A.6.
...
**Still open on this screen:** ~~saved maps in the map-source picker~~ — ✅ **BUILT 2026-09-04
as 16.0**: the Map picker lists every map `SavedMaps` finds across §3.3's three roots below a
`Saved` separator, picking one loads the file into `_data` and locks the seed controls (a seed
describes a generated map and means nothing to a file), and **`can_start()` gained rule 7** —
a lobby may not seat more players than an authored map was written for, because
`MapGen.build_from()` hands a player their base only from the entities the map lists for their
index. The OPEN slot role, which needs 12.1's listening host; and the art — it is
...
- **`TERRAIN_COST` is on a base-10 scale** so slower ground stays integral (the sim is all ints,
  §7.1). `SimMap` is folded into `state_hash()`, without which clients disagreeing about terrain
  would hash identically.
- **`PathService` holds one A\* grid per `Domain`.** It held one — land's — and a ship routed
  against it sailed up the beach. `ProductionSystem` asks for a spawn tile in the **trained
  unit's** domain rather than always LAND.
- ⛔ **A DECLARED ROW IN A TABLE IS INDISTINGUISHABLE FROM A WORKING ONE.** `DOMAIN_TERRAIN[AIR]`
  listed rock and forest from 2.1, so air *looked* finished, and it took three separate fixes to
  become true: the rig had to arrive (`speed` was 0); `is_terrain_passable` tested `move_cost` —
  `IMPASSABLE` for both — **before** it reached the domain, so the row was unreachable code; and
  `PathService` builds a grid per domain lazily, so **nothing had ever built an air grid** and the
  first flight re-swept every grid mid-match (22 ms against a 15 ms budget). Nothing caught any of
  the three because no air unit had ever asked for a route.
- **`despawn()` frees tiles BEFORE dropping the entity**, or occupancy keyed by id leaves tiles
  claimed forever.
- ⛔ **A BUILDING'S `pos` IS ITS FOOTPRINT CENTRE**, not its origin tile, so the view draws every
  entity identically. This premise broke three rounds of cliff-collision fixes — see §11.10.
- **`MapValidator` HAS A SECOND CONSUMER IN ANOTHER GODOT PROJECT** (16.4b). MapMaker carries a
  verbatim, hash-checked copy in `MapMaker/format/` and runs it on every save, so these rules are
  the tool's answer as well as the lobby's — retune `MIN_NEARBY`, `NEARBY_TILES` or `SHORE_TILES`
  and the tool refuses to save until it is re-copied, by design. `enum Type` is in the guarded set
  too, because `_is_sea_map` reads it and a renumbered enum would make the tool call sea maps land
  maps silently.
- **`MapGenerator` places the dragon nest**, and that is where *"one dragon per map"* is enforced.
  There is no cap anywhere else.
- **Starting resources are placed FOR THE SCREEN as much as for the grid**: iso sends `dx-dy` to
  screen x, so "below the town centre where the map is empty" is behind the HUD.

#### 11.2 Map generator (2.4b)

Three sim classes. **`MapData`** is the intermediate representation — terrain bytes plus an entity
list — and it exists so that one thing serves four: what a generator produces, what a saved file
holds (2.4c), what the skirmish screen previews (1.6), and what `MapGen.build_from()` turns into a
world. A generator writing straight into a `SimWorld` can be none of those. **`MapGenerator`**
produces one; **`MapValidator`** is the gate, and is built into `generate()` rather than offered
beside it. The owner's `game_map_gen/` prototype is **left untouched** — it is theirs.

Sizes: 2P 96×96, 4P 128×128, 8P 192×192. *(`map_generator.gd`'s own header says 184 for 8P; the
code computes 192 and this document is correct — the reverse of the usual direction.)* Judge a
layout with `dev_preview/preview_mapgen.tscn`, which writes one PNG per type. **It is the only way
to judge a map layout** — there is no assertion for "does it look right".

**THE RULES THAT BIND ANYTHING TOUCHING THIS GENERATOR:**

1. **SIZE IS BY AREA, NEVER BY SIDE.** `side = 64 * sqrt(players)`. Growing the side linearly
   gives 8 players 64× the area of 2, and **fog is one byte per tile per player** — a 300×300 map
   takes one tick from 12 KB to ~100 KB and one player's stream to ~1 MB/s. This is a wire-format
   constraint wearing a level-design costume.
2. **STARTING RESOURCES ARE PLACED, NOT SAMPLED.** The validator's first run caught a desert start
   with one reachable tree and no food at all. Hoping the noise put a wood nearby is how a player
   gets a start they cannot play.
3. **CONNECTIVITY IS A HARD GATE, not a warning.** Flood-fill from every start: each must reach
   every other start and a minimum of each resource, else regenerate. It is also what makes the
   generator headlessly testable. *Revisited for Archipelago, where the claim had to change rather
   than relax — §11.6.*
4. **THE MAP IS SENT to joining clients, never regenerated by them** (§12.1 step b), because
   `FastNoiseLite`'s float maths is not guaranteed identical across CPUs.
5. **WILDLIFE IS PLACED PER START, NOT PER MAP.** A fairness choice, not a literal reading of "two
   per player": scattering 2n animals over the board hands them all to whoever the noise favours.
   Every placer here is **best-effort** — it skips claimed and unwalkable ground and gives up after
   a bounded number of tries, because a start hemmed in by water should yield fewer sheep rather
   than fail the whole map.

➡️ **OPEN: THE TWO DIAGONAL RIVERS ARE STILL REMOVED, AND THE REASON THEY WERE REMOVED IS GONE.**
They went because a footprint is an axis-aligned rect — the same limit that has
`wall-facings-reachable` blocked. A crossing is now a **terrain byte** (`BRIDGE_X` / `BRIDGE_Y`)
rather than a footprint, so that limit no longer applies to a bridge at all, and the art has
carried all 8 directions throughout. Restoring them is now an ordinary generator change. Left
undone deliberately: nobody has asked for diagonal rivers.

📝 **A design question rather than a defect:** `_bridge_positions` spaces crossings at
`(i + 1) / (count + 1)`, so an **odd** count always puts one at `along = 0` — and a two-player map
puts both starts at `along = 0` too. On every 1-crossing and 3-crossing map **the two players face
each other directly across the central bridge.** Biasing the crossings off-centre is the lever if
that is not wanted.

#### 11.10 Plateaus and cliffs — `CliffPlan`, and why collision is a terrain byte

A plateau is **one drag** in the MapMaker, in either of two shapes: the one that looks like a
square on screen (a rectangle in `(u, v) = (x + y, x − y)`) and the one that looks like a diamond
(a rectangle on the tile grid). The tool lays the whole ring of ~40 pieces from it.

⛔ **THE RULES LIVE IN `CliffPlan` ONCE AND EVERY CALLER ASKS.** Three consumers must agree about
which piece goes where — `preview_cliff_variants`, the MapMaker's plateau tool, and whatever the
generator grows next — and **three implementations of a rule the owner checks by eye is how the
corners went missing twice.** It takes a SET OF TILES rather than a `Rect2i`, which is what lets one
function serve a grid-aligned plateau, a screen-aligned one and an arbitrary painted blob; the shape
only ever enters through `high.has(t)`.

⛔ **ONE LENGTH PER RUN, AND IT IS NOT A GREEDY FILL.** `WallPlan` covers a wall longest-first — 12
tiles as 9 + 3. A cliff must not: **every piece is a window onto ONE continuous 16 m strip of rock
and every window opens at `u = 0`**, so two pieces of different lengths restart the rock at
unrelated places and the silhouette steps sideways. The largest rung that divides the run EXACTLY
wins, and a run no rung divides is laid in 1-tile fillers — which is the stamped look, and why
`runs_of` reports the length it chose so a tool can tell an author to resize.

⛔ **WHERE A PIECE'S ROCK FALLS IS MEASURED, NEVER DERIVED, AND THREE HAND-DERIVATIONS SHIPPED
WRONG.** The arithmetic was fine every time; the premises were not:

- **a sprite is anchored at its footprint's CENTRE**, not the tile it stands on, so an offset
  counted from the origin tile is already shifted before any projection (§11.2's 2.3 rule);
- **the rock is two tiles wide, and a diagonal's is three** — it was modelled as the single `(1,1)`
  line, which is the column the scout was standing *beside*;
- **`AXIS_Y` is not `AXIS_X` mirrored.** Its band starts a tile to `+x`, so blockers were laid on
  tiles the rock never touched while the touched ones stayed open.

⚠️ **AND NONE OF THE THREE WAS CAUGHT BY A GREEN SUITE, BECAUSE THE FIX, ITS REGRESSION TEST AND
ITS DIAGNOSTIC PROBE ALL CARRIED THE SAME PREMISES** — three checks wearing one hat. The structural
answer is that they now share ONE table and **`test_the_cover_tables_are_what_the_atlas_says`
re-measures it from the staged atlas every run**, so a re-bake that moves an anchor fails a test
instead of shipping a walkable cliff. Re-read the tables any time with
`probe_saved_map.tscn -- coverage`. *(A probe that has never failed has not been shown to be able
to: negative-control it by disabling the fix.)*

⛔ **A CLIFF'S ROCK BAND IS A BYTE IN `SimMap.terrain`, NOT AN ENTITY PER TILE.** Correct collision
first cost 621 invisible 1×1 blockers on one hand-drawn map — against 292 of everything else — and
**no single system was at fault**: vision, population, combat, death, gather, separation and
garrison each paid a slice for entities that do nothing. Measured with `probe_map_cost.tscn`:
**9.20 ms a tick against 3.12 ms, and a 336 ms world build against 52 ms.** The only question a
blocker answers is *"may a unit stand here"*, and `Terrain.ROCK` already answers exactly that —
impassable for land and water, **crossable for AIR** (a dragon flies over a cliff), in the
pathfinder's cost table, excluded from `is_buildable`, and free, because the byte array is the same
size whether a tile is grass or cliff.

⚠️ **THE CONVERSION IS ON LOAD, AND THAT IS THE POINT.** `MapGen.build_from` paints the byte and
skips the spawn; `CliffPlan` still plans blockers, MapMaker still writes them and `MapValidator`
still sees them — so **every map already saved gets this without being repainted.** The thing that
would silently undo it is somebody tidying up by making MapMaker stop writing blockers, at which
point old maps quietly become walkable again.

📌 **Godot's navigation and collision LAYERS are a physics-server feature this sim deliberately does
...
rather than a placement fault; wants an art-side answer before code.

📝 **A screen-horizontal cliff line is constant `x + y`** → `AXIS_D2` → built entirely of `_diag`
pieces; a screen-vertical line is constant `x − y` → `AXIS_D1`. Getting that backwards is what hid
the diagonals' missing collision for a whole round.
...
⏸️ **THE BUTTON IS PARKED, AND SPLIT IN TWO — THE OWNER'S RULING, 2026-09-04.** *"lets park the save
map button for later, or build it in 2 sub phases, Save Map - only seed and spawndetials for a map
layout you liked while you where playing it. and Save Game - when a map is interupted by IRL so you
can pick it back up with your friends or vs AI later."*

**The split is this document's own line and is hereby reaffirmed by the owner rather than
introduced:** Save Map is 11.3, Save Game is 12.4, and the third bullet below has warned since it
was written that the button must not blur them. Nothing in Phase 16 waits on either — 16.0 was
scoped down to the picker alone, and 16.4a reads the five maps that already exist on disk.

✅ **AND THE "HOW" IS SETTLED: IT WRITES A `MapFile`.** The owner's clarification, 2026-09-04, when
this document read *"only seed and spawn details"* back to them as a design constraint: *"it was
just my way of saying i want to be able to save the file not guidance on how it should be done, use
what what ever way allows loading what you are playing, with as little effort as possible as you
will be ingame at the time."*

So **`MapFile.save()` is the answer on both criteria at once**, which is worth writing down because
the cheap-looking option was the expensive one: writing `map.png` + `map.json` from a match is
already implemented and covered by eleven tests, while a seed+spawns writer is code nobody has
written and a **second format** beside `MapFile` (decision 1's hazard). It is also the only one of
the two that satisfies *"loading what you are playing"* — `MapGenerator` is `FastNoiseLite`, so the
same seed produces a **different** map after any generator change or on another machine, which
`dev_preview/preview_author_maps.gd`'s header calls *"the whole reason the file exists"*. **The
seed still rides along, inside `meta`, as provenance** — which is what that field is for.

⚠️ **A LESSON ABOUT READING THE OWNER'S WORDS, kept because it cost a round trip:** *"only seed and
spawn details"* was a description of the FEATURE they wanted, not a specification of its
implementation, and it was treated as the second. Where an instruction can be read as a constraint
on internals, check whether the outcome is what was actually being asked for.

The feature, when it thaws: a Save Map button on the pause menu, so a player who likes a random map
— or one someone else made and shared — can name it, keep it, and pick it again from 1.6. Three
...
| **4.8c** | ✅ **DONE 2026-09-07 — GARRISON IS NOW A CAPABILITY. Siege engines and dragons may not garrison.** Owner's ask 2026-09-06, and again on 2026-09-07 — *"dragon cannot garison"* — when 11.2's trophy became movable the same day and made *"put the dragon in the castle"* the safest move in the mode. Until this they could, and `GarrisonCommand.validate()`'s header said so in as many words: *"Here EVERY unit can garrison — there is no unit-side capability to fail — so a bad id in the list is a client bug or a stale selection, not a mixed group."* This row adds the capability that sentence says does not exist. **`UnitDef.can_garrison`, default true**, false on the four siege engines (`siege_ram`, `ballista`, `onager`, `trebuchet`) and both dragons (`dragon`, `dragon_baby`).<br><br>⚠️ **DO NOT DERIVE IT FROM `trainable_at == building.siege_workshop`.** It looks like the free version and it is wrong twice: **the dragon trains nowhere**, so the derivation misses the half of this row that prompted it, and it pins a rule to a *training location* — the shape `MapGen.DEBUG_ENEMY_SQUAD` paid for, where a placement pinned to a literal goes quietly wrong the day the thing it was chosen against is retuned. An explicit field is one line in five defs and cannot be wrong quietly.<br><br>⚠️ **THE MIXED SELECTION IS THE REAL WORK, AND IT CHANGES `validate()`'s CONTRACT RATHER THAN ADDING A CLAUSE.** Ten swordsmen and one onager ordered into a tower must **admit the ten and drop the onager**, not refuse the order — which is already 4.8's own rule for a building that fills up mid-walk (*"the fifth is refused there rather than being promised a slot here"*). A blanket refusal would make one siege engine in a box-select cancel a garrison the player plainly meant. So the capability is filtered in the ORDER and re-checked on ARRIVAL, the same order-then-arrive split the command already uses.<br><br>**Two enforcement points, not one:** `GarrisonCommand.validate` because the server is the only trust boundary, and `SelectionActions` so the button is not offered for a selection that is all siege — the arrangement the placement ghost and `PlaceBuildingCommand` already share. **Leave the transport rule alone**: a carrier already refuses to be cargo through `garrison_cap > 0`, which is a different rule for a different reason, and collapsing the two would say a transport cannot garrison *because it is siege*.<br><br>✅ **WHAT SHIPPED, and the plan above was right about every part of it except where the client gate lives.** `UnitDef.can_garrison` default true, explicit `false` on the six defs, filtered in `GarrisonCommand.validate` (which now returns true when **any** named unit may go in and false only when none may), filtered again in `apply`, and refused a third time in **`SimWorld.garrison_unit`** — which this row did not anticipate and is the one that matters most: it is the single gate *every* route in passes through, so the rule is true of the STATE rather than of one command. `GarrisonSystem`'s arrival, a transport boarding and anything written later all end up there.<br><br>⚠️ **THE CLIENT GATE IS `GameView.tap_action`, NOT `SelectionActions`.** This row named the wrong file, and `selection_actions.gd`'s own header says why: the garrison ORDER is issued by a **tap on the building**, so the `&"garrison"` action in that file is the building's occupant roster (who is inside, and letting them out) and never the order. The gate is a fourth argument, `has_garrisonable_selection`, **defaulted true** so every existing caller and test reads unchanged — a dragon is perfectly *movable*, so it could not be folded into `has_movable_selection`. A lone dragon tapping its own castle now **reselects**, which is the invisible-refusal failure `tap_action`'s own comments warn about, avoided at the exact line that warns about it.<br><br>**Eight tests**, including the two the ask really turns on: the six defs asserted **by name** so one quietly losing the field fails with its own id, and the mixed order admitting the archer while dropping the onager *and* leaving the onager not walking toward a building it can never enter | |
...
| 4.10 | ✅ **Special abilities + cooldowns** (2026-08-29). `AbilitySystem`, `AbilityCommand`, and an `ability` block on `UnitDef`. **Two units carry one, and both were promised before they existed**: the MONK heals (IDEA.md 4.6's *"healed by a friendly healing unit"*) and the DRAGON breathes fire (13.1's *"area damage"*). So this is two sentences arriving rather than new design, which is also why it is these two and not a set invented to fill a system.<br><br>**THE MONK HAD NO VERB AT ALL BEFORE THIS**, and that is the strongest argument for the shape it took: 100 gold, `attack.damage 0`, no `gather_rate` — the one unit in the roster that could be trained and then only walked about. 6 hp per 20 ticks, range 4, and it **keeps working until the target is full** rather than needing a press per tick.<br><br>**`id` IS THE SWITCH AND `effect` IS THE BEHAVIOUR**, kept apart deliberately. One `amount` field cannot mean "hp restored" for one and "damage dealt" for the other unless something says which, and keying that off the id would make every future ability an arm in `AbilitySystem` instead of a JSON entry. There are exactly two effects, `heal` and `damage`.<br><br>**The same order-then-arrive split `BuildCommand`/`BuildSystem` uses**: `Task.ABILITY` is a travel task, so a monk told to heal somebody across the square walks over. Leaving it out of `is_travel_task()` is the exact failure `set_task_garrison` hit in 4.8 — PathService drops the route and the unit stands in that task forever — so there is a test on the predicate itself.<br><br>⚠️ **`ability_target_tile` IS THE AIM AND `task_target_tile` IS NOT.** `set_path` rewrites the latter to wherever the route could actually end (4.1), so a breath weapon reading it lands on the dragon's own feet. Two fields, and the second exists only for this.<br><br>**Not built, and named so nobody looks for them:** no resource cost (IDEA.md 4.10 allows one; nothing in the roster has one, and the place it goes is `AbilityCommand.validate`), no auto-use by anything including the AI, and **no friendly fire** — the blast tests the caster's own war predicate, so a dragon cannot burn its own army *or* a herded flock (4.9's bug, at a third mechanism). **There is no fire-breath ART**: `vis.dragon` has one clip, `static`, and `asset_request.md` [P7] carries the ask. ✅ **DELIVERED 2026-09-04** — `vis.dragon_rigged` ships an `attack` clip (0 A.D.'s `shootFire`, the dragon rearing and breathing) alongside `idle`/`walk`/`die`/`decay`. It is the same anim name every other unit uses, so nothing here changes but the `visual` id. ⚠️ **One clip serves both weapons**: the rigger was asked for a melee `attack` and returned a fly cycle instead, so `shootFire` covers the 30-damage attack AND this ability until that is re-rigged.<br><br>⚠️ **THE BLAST FALLS OFF BY RING SINCE 2026-09-07, and the ability that had been 40 FLAT is now 250 at the centre.** Owner, after play-testing the mother in HowToPlay scenario 5: *"the dragon special attack is weak, it needs a aoe o 5x5 with 250 damage to any unit or building around the click with -50 damage every ring out."* **The 5×5 was already right** — `ability_radius: 2` has meant `2 * 2 + 1 = 5` tiles across since this row shipped, and `BlastEffects` is sized from it — so what changed is one new field, `ability_falloff_per_ring: 50`, and the amount: **250 / 200 / 150** by ring, which is the three nested squares of the owner's own diagram.<br><br>**RINGS ARE `CombatSystem.tile_gap`, which is the only honest choice and not merely the convenient one.** It is Chebyshev, so the rings *are* the concentric squares the blast rect already was, and it measures to a **footprint**: a town centre with any part of itself on the aim tile is in ring 0 and takes the full 250. A centre-to-centre measurement would put an 8×8 town centre in ring 4 of its own doorstep and out of the blast entirely — the exact bug `tile_gap`'s own header records having fixed for melee reach. ⚠️ **A ring the falloff reduces to nothing is SKIPPED, not floored**: `_damage_after_armour` enforces `MIN_DAMAGE`, so handing it a 0 would deal the floor to something the falloff had just placed out of range. Unreachable at the dragon's numbers (its outer ring is 150) and one line to be right about for whatever asks for an area effect next.<br><br>**It is a 6× change at the centre and the 120 s cooldown is what makes it sane** — 250 kills anything below a knight outright and takes a third off a town centre, but she fires it about once a fight. The two numbers were retuned in the same week and neither reads correctly without the other. ✅ **NO FRIENDLY FIRE, AND IT IS NOW A DECISION RATHER THAN A READING** — owner, 2026-09-07: *"confirm no friendly fire."* *"Any unit or building"* means both **kinds** of thing, not *including your own*. Worth asking because 250 makes it expensive to be wrong about: a 5×5 that burned your own side would wipe a stack of your own infantry in one press, and at a 120 s cooldown a player would learn that once and not again for two minutes. The switch, if it ever moves, is `_is_hostile_to` and nothing else | |
...
| 5.3 | ✅ **BUILT 2026-09-03, awaiting the owner's play-test** (`673540b`; card in `Test`, not `Done`, because the check that can judge it is somebody playing it and they asked for it by name). **Eight new upgrade links, length for length** — short→short, medium→medium, long→long, gate→gate, across wood→stone→reinforced — on top of 5.8's three gate conversions. The shape is the owner's: a long wall now carries **four** action tiles, two of them upgrades (Palisade Gate, Stone Wall, destroy, repair), each cropping the art of the thing it gives you.<br><br>**`upgrades_to` IS A LIST**, and the long wall is why — one field could not say "gate OR next tier". The order is the tile order, so the gate stays first: a button that moves under a player's thumb because a new one appeared above it is the change nobody asked for. A bare string still parses as a one-target list, because this project's owner has now proved that people hand-author its JSON (15.8).<br><br>**THE COMMAND CARRIES ITS TARGET** — one building offers two and the server cannot guess which tile was pressed. `resolve_target` is shared by the panel and by `validate()`, and **the list is the authority**: a client naming `building.castle` is refused. That is a real exploit, not tidiness — a 36-wood palisade would otherwise become a 650-stone castle for the difference.<br><br>**THE CREDIT CROSSES RESOURCE KINDS at 1:1**, the owner's call, and that rate is **read off the game** rather than chosen: `market.json` prices food, wood and stone identically against gold (130/70 each), so one-for-one is the only exchange rate this game has ever declared. Routing it through the market would make an upgrade depend on owning one and lose 60% to the spread. It counts the **surplus** of a kind the target still wants, not only the kinds it wants none of — the palisade gate needs that. A long palisade → stone wall is **9 stone** (45 less 36 of credit), where same-kind credit alone would have charged the full 45, which is what a rebuild costs and therefore another way of saying there is no upgrade. **50% health stays 50%** — already `convert_building`'s rule, now tested against a tier upgrade and not only a gate.<br><br>⚠️ **NO WATCH TOWER → GUARD TOWER**, the owner's call on the footprint mismatch ([3,2] vs [3,3]). Nothing had to be removed: `validate()` has refused a footprint change since 5.8, because `convert_building` keeps the ground the building already holds. Pinned by name in a test — it is the pair anybody reading the roster proposes first — plus a second test that walks **every** building and asserts every declared target keeps its footprint.<br><br>⚠️ **AND THE "A TIME" REQUIREMENT DIED WITH THAT TOWER.** The reasoning below was about a NON-WALL upgrade; every upgrade that remains **is** a wall, and a wall is exactly the case where instant is right — a timed tier upgrade would open your own defences for the length of it. Said out loud in `UpgradeBuildingCommand`'s header, with the note that 9.3's queue is where a timer goes the day something that is not a wall upgrades. Suite 2158/0. `preview_walls` photographs the action row and prints what is on it — **at age 3**, because at age 2 a long palisade correctly offers its gate and nothing else | |
...
| 6.6 | ✅ **A HUNTER EATS WHAT IT KILLED** (2026-09-06, `0e1e3d2`). Owner: *"after a villager kills a sheep the next action is harvest sheep, not kill another sheep."*<br><br>⚠️ **MEASURED BEFORE IT WAS EXPLAINED, BECAUSE THE FIRST TWO GUESSES WERE BOTH WRONG.** The tap path was suspected first and is **fine** — a tap on a live sheep resolves to ATTACK, before and after a carcass appears beside it. What a real villager actually did, driven in the sim: killed sheep one at tick 155, re-acquired sheep two at tick 160, re-acquired sheep three at tick 340, and finished with **two carcasses on the grass and zero food gathered**. One hunt order became a massacre that fed nobody.<br><br>The bug looked exactly like combat working correctly. `Diplomacy.is_enemy` says a gaia **unit** is fair game — correctly; that is what lets anybody hunt — so a sheep is an enemy and `_reacquire` did what its own header says it is for: *"an army kills one unit of a group and then stands among the rest doing nothing."* That is a **fight**. A hunt is the other thing, and the difference is not the target — it is **whether the killer can eat what it killed**. So the test is the killer's own `gather_rate` and **no unit is named anywhere**: a villager claims the carcass, a swordsman re-acquires exactly as before, and a future butcher inherits this by having a rate.<br><br>⚠️ **IT WRITES TO THE PREY, NOT THE HUNTER, AND THAT IS THE TICK ORDER.** The carcass does not exist when the blow lands — `DeathSystem` runs last and is what spawns it — so there is no id to hand over yet. `CombatSystem` leaves `SimUnit.killed_by_id` on the body, `DeathSystem` cashes it the moment the node is real, and `GatherSystem` (third in the order) acts at the top of the next tick: a tenth of a second between the sheep dropping and the villager kneeling over it. Four refusals, and the last two are rules rather than guards — **nobody claimed it** (a starved animal, a debug destroy, every carcass `MapGen` places: all still lie there for whoever wants them), and **the hunter is only given it if still IDLE**, because `CommandSystem` runs first and a fresh order this tick is already on them; overriding it would be the game countermanding the player a tenth of a second after they tapped | |
...
| 8.6 | **Chat** — wireframe only (§8.2b). The transport is unbuilt and the design question is per-team versus all-players. ⚠️ **A second, smaller question arrived from a playtest before the first was answered** (`8.x-chat-align`, 2026-09-20): the owner wants message text **right-aligned**, and which of the two readings they mean decides the work — *every* message right-aligned is one property, while *mine right and theirs left* needs the sender compared against the local player and is a fact the panel may not hold yet.<br><br>✅ **CLOSED 2026-09-21 (`c47f5b0`), AND IT WAS NOT THIS PANEL.** The owner's answer was *every* message, and the target was the **in-match `MessageLog` above the minimap** — the block printing the disconnect notices — not the CHAT page this row is about. One line in `MessageLog.say()`. Worth keeping for the measurement rather than the fix: the block sits at `-WIDTH - GameScene._MINIMAP_MARGIN` and the minimap below it is `AREA_SIZE` square at the same 12 px inset, so **both right edges land on `viewport - 12`** and the text now hangs off the edge it shares with the map. The per-team versus all-players question is still open and still belongs to the transport | |
...
| 8.4 | ✅ `NoticeToast` | `[MVP]` |
| 8.5 | ✅ `PauseMenu` — stops `SimClock`, so a real pause rather than a panel over a ticking match. **Reached from the SETTINGS corner button since 2026-08-21**, not from a pause button in the age header: that button was the project owner's call to retire, and its actions belong beside the three sibling pages rather than in the top-centre chrome. Still the same class — Resume/Resign/Quit is still what it holds, plus **SAVE & EXIT** since 12.4 | `[MVP]` |
| **8.9** | ✅ **BUILT 2026-09-21 (`085f83c`) AND CONFIRMED IN PLAY BY THE OWNER** (board `8.x-build-menu-modal`, Done). The design below is what shipped and is kept in full, because every trap in it is still live in the code. **THE DETAIL GRID BECOMES A PAGE** (owner, 2026-09-20). Buildings, upgrades and a new **units** view move out of the 4×3 strip and onto a **fourth `HudPanel` page** beside CHAT, TECH TREE and MARKET — *"a pop up modal like we have for voice chat"*. **The action column (`MAX_ACTIONS`) stays exactly where it is**; only the detail grid (`MAX_DETAILS`) moves. The clock keeps running underneath, inherited from `HudPanel` rather than decided: a build page that paused the match would be a pause button wearing a hammer.<br><br>**Two exits and they differ.** An upgrade or tech tile queues as it does today; a **building** tile **closes the page** and hands over to the existing drag-to-place flow, because placement is a gesture on ground the page would be covering.<br><br>⛔ **TILES KEEP THEIR CURRENT SIZE AND THE `>` PAGER STAYS** (owner: *"scale the icons exactly as is… nice and big and easy to read"*), which is what makes this structural rather than cosmetic: **`MAX_DETAILS` stops being knowable at compile time**, since a fixed tile in a page whose size follows the viewport fits a different count on a desktop window and a phone. `page_of()` must be **told** the slot count — never read a `Control.size` from inside `SelectionActions`, which is pure and node-free precisely so the action model can be asserted headlessly.<br><br>⛔ **UNITS GET A PAGE TOO, AND IT TRAINS.** A new `cat_units.png` tile opens it (the icon exists in `assets/UI_Gen/sliced/icons/` and needs copying into `game/assets/ui/icons/` — no bake). Per-unit train tiles **stay in the column where they fit and collapse into that one tile where they do not**, which takes a castle from 4 tiles to 1 and brings it back under the cap. ⚠️ **The collapse must be decided from something STABLE** — the building's whole roster at its owner's age — because what a building emits varies with age, rally point, garrison, gate and techs, and computing it from the live count would re-arrange the column under the player's thumb the instant a rally point is set. `_building_actions` already refuses to do that to people and says why.<br><br>⚠️ **TWO ROUTES TO ONE COMMAND**: the page must ASK `SelectionActions` for affordability, pop cap and age gate rather than re-deriving them — §5's split-predicate trap, which cost this project a soldier shooting an ally's barracks. And §4 regardless: if either UI greys it, `TrainCommand` still refuses it | |
| **8.10** | ✅ **BUILT 2026-09-21 (`5450eff`)** (board `8.x-destroy-confirm`, Done). **A CONFIRMATION BEFORE DESTROYING YOUR OWN UNIT OR BUILDING** (owner, 2026-09-20): *"currently it's insta death or destroy, sometimes by accident."* A modal before the command is **sent**, naming what and how many. ⚠️ It is a HUD affordance and **not** a rule — the command stays exactly as trustworthy as it is today (§4). ⚠️ And it is a new Control inside a match, so `preview_touch_controls` applies: `emulate_mouse_from_touch` is off for the length of a match and anything that is not a `BaseButton` is inert under a thumb. **A confirm dialog nobody can dismiss on a phone is worse than no dialog.**<br><br>⛔ **RELATED AND PULLING THE OTHER WAY: `8.x-action-column-overflow`.** `_capped()` slices the action column's tail at `MAX_ACTIONS` 8 and the castle already emits nine with a rally point set — so **Destroy is the tile being dropped**. A confirmation in front of a button that is sometimes not there at all is the less useful half; do them in one sitting.<br><br>✅ **AND THE OVERFLOW TURNED OUT NOT TO NEED A FIX — 8.9 HAD ALREADY REMOVED IT** (closed 2026-09-21 on the owner's ruling, no follow-up card). Moving the detail grid out of the strip took the castle back under the cap, so nothing is sliced today. ⚠️ **What the investigation found is the part worth keeping: there is now ZERO HEADROOM.** The worst case measured is exactly `MAX_ACTIONS`, so the *next* action added to any building re-opens this silently — `_capped()` still slices without a word. The owner's decision was to leave it there rather than file it, which is a choice and not an oversight | |
...
| 11.2 | ✅ **BOTH ARE BUILT NOW. TROPHY 2026-09-07, KING OF THE HILL SINCE** — the zone, `SimPlayer.score` per side, the win, the standings panel (`11.x-koth-hud`) and a sound on a change of control. ⛔ **SO EVERY SENTENCE BELOW THAT SAYS KotH "STILL WANTS" THE ZONE, THE SCORE FIELD OR THE RING IS SPENT**, and they are left standing only for the reasoning around them. **What is still declared and inert is REGICIDE**, plus the wonder (`11.x-wonder-victory`); the other three conditions are live and none is greyed. Trophy wanted a `unit.dragon_baby` def, a MapGen that gives every player one, and an `is_trophy` flag rather than a hardcoded id — all three exist, and **none of them was the one this row expected to be hard.** The def landed with 13.2b. *"A MapGen that gives every player one"* is **not** `MapGenerator` arithmetic (see the 13.2a note below, which got this wrong in good faith): a trophy wants to be as **near** its owner's base as possible, the exact opposite of a nest needing to be far from everyone's, so it is `MapGen._place_trophies` walking `_next_free_tile` out from the town centre — a post-build pass beside `_place_ai_handicaps`, which already existed. `UnitDef.is_trophy` replaced `WinConditionSystem.TROPHY_DEF_ID` (which was `unit.dragon`), so the rule now names no dragon at all.<br><br>⚠️ **THE COLLISION THIS ROW'S WARNING DID NOT COVER, AND IT WOULD HAVE BEEN CATASTROPHIC AND SILENT.** A trophy is a `unit.dragon_baby` — **the same def 13.2 hatches at a nest**, for an opposite job. `NestSystem` collected every hatchling in the world **by def id alone**, and `_reap_orphans` kills any it finds that no nest has recorded. A trophy is recorded on no nest, so every trophy would have been killed on tick 1 and, since losing your trophy is defeat, **the entire lobby would have been eliminated immediately — on a map with no nest on it at all**, because the reap does not need one to fire. `owner_id == 0` in that collection pass is the whole fix, sound for the reason `NestSystem`'s own header already gave (*the hatchling is gaia's until it grows*) and pinned from **both** directions, since narrowing a reap must not turn it off.<br><br>⚠️ **INERT UNLESS EVERY PLAYER ACTUALLY GOT ONE, and then CONQUEST rather than nothing.** Placement is **all or nothing** and `SimWorld.trophy_def_id` — a world field, deliberately not hashed, identical by construction — is written only on success and is what arms the rule; the roster flag answers *which unit is the trophy*, the world field answers *is there a trophy match here to decide*. Reading the flag directly would arm the rule on any map, including the debug map where only player 1 has a base. **An unarmed trophy match falls back to last-man-standing**, which the first version got wrong: deciding nothing skipped the elimination rule too, so a misconfigured match was **unendable** — two players could wipe each other out and stand in an empty world forever. *A hang is not the safe direction; it is a slower way of being broken.* Found by reading a stale test that was still passing for a reason its comment no longer described.<br><br>⚠️ **AND ENABLING THE LOBBY PICKER FOUND A CONTROL WIRED TO NOTHING.** `_mode` was written in exactly one place in `skirmish_screen.gd` — a joined client adopting the host's config — so **nothing ever connected `_mode_picker`** and a host selecting a victory condition changed nothing; `build_config()` always returned `LAST_MAN_STANDING`. It could not have been noticed while the other two items were disabled: a dropdown with one selectable entry cannot demonstrate that choosing does nothing, and `_mode`'s default is what that one choice would have set. **The same defect as the selected-units roster and the minimap corner buttons.**<br><br>**TWO OWNER DECISIONS FROM THE FIRST PLAY, both 2026-09-07.** *"No second dragon on trophy map type, only 1 per player"* — `MapGen._clear_dragon_nest` despawns the gaia nest and its mother, guarded on `w.trophy_def_id` and **not** on `cfg.mode`, so a match that failed to arm keeps its nest and plays as an honest conquest. The mother goes with the nest because a `guards_post` animal left behind re-homes on some unrelated rock and then pays out nothing when killed. And *"it can stay small but let me move it around"* — `unit.dragon_baby` **speed 0 → 55**, the slowest mobile unit in the game, priced against `domain: air`: it crosses rivers, walls and gates, so a trophy can be flown out of an encirclement and crawling is the compensation. ⚠️ **The zero was never what kept 13.2's hatchling still** — that one is gaia's, every move command is owner-gated, and its roam radius is 0 — so the claim is untouched, and now asserted rather than implied. **Its mobility is also what prompted 4.8c's second half**: a movable trophy could reach a castle, and *"put the dragon in the castle"* would have made a mode about defending a place into a mode about parking.<br><br>📝 **THE TROPHY DOES NOT GROW UP, AND MUST NOT.** Owner-observed and correct: growing REPLACES the hatchling with a `unit.dragon`, which carries no `is_trophy`, so its owner would hold no trophy on the next tick and be defeated **because their dragon succeeded**. Guaranteed by two unrelated filters (player-owned, so `NestSystem` ignores it; recorded on no nest, so no claim can mature into it), which is exactly the kind of implicit promise that gets tidied away — so it is asserted past the full `GROW_TICKS` window.<br><br>📝 **STILL NOT FIXED AND NOT FIXABLE HERE: every trophy is the same picture.** `vis.dragon_baby` carries no `colours` flag and `visuals.json` records the dragon as bespoke art with no playercolour mask at all, so in an 8-player trophy match the trophies are indistinguishable. Position (each at its owner's base), the selection ring and the minimap blip are the cues that do work. This is 13.x's ownership-cue problem arriving in the mode that needs it most, and the art side has already ruled it unfixable at any price.<br><br>KotH still wants the zone as **map** data, a per-player score (deliberately not added — an unwritten field that reaches the HUD is exactly the hole 4.11's counter was), and the minimap ring. **Inert is the safe direction to be unfinished in:** "you lose when your trophy dies", on a map with no trophies, defeats everybody on tick 1 — *the sentence that kept this row unbuilt for the life of the project, and it was right*. It is **still exactly true of a match where placement failed**, which is why the arming guard above got more assertions than the win condition it guards. Also unbuilt: regicide (the Celtic heroes would serve), capture the flag, wonder — and regicide is now **a line of JSON**, since `is_trophy` names no dragon.<br><br>⚠️ **KotH'S RULES ARE SETTLED — the owner's scoring ladder AND its target, 2026-09-01, in §11.9.** Scored **per side**, and the target is worked back from **five minutes uncontested** to `KOTH_TARGET_SCORE = 9000` (the shipped 1000 is 33 seconds). It **reverses** what `_king_of_the_hill()`'s header says the rule is. The zone, the score field and the ring are still the three missing pieces; what is no longer missing is any design question.<br><br>⚠️ **AND 13.2a BUILT MOST OF WHAT BOTH ROWS WANTED FROM MapGen, 2026-09-04.** *"A MapGen that gives every player one"* and *"the zone as **map** data"* are the same machinery from this side of the fence: `MapGenerator._place_nest` sites a gaia POI, keeps it clear of every start and off the roads between them, and could as easily place one per start as one per map — see `nest_start_clearance()` for the derivation and the three defects that shaped it. **What Trophy actually still lacks is sprite scaling for a 20% baby, which is an `EntityView` change**, and what KotH lacks is `SimPlayer.score` and the ring. *Neither is the map-generation problem these rows were written expecting*. ⚠️ **AND THE FIRST HALF OF THAT WAS WRONG, WHICH IS WORTH LEAVING IN.** `_place_nest` was the wrong precedent: it *could* place one per start, and it *should not*, because a nest is kept clear of every start and off the roads between them — the hard arithmetic that took three defects to get right — while a trophy wants the opposite and needs none of it. The sprite scaling did land with 13.2b as this note predicted; the machinery did not transfer at all | |
...
| # | Item | Tag |
|---|---|---|
| 12.1a/b | ✅ Hosting, joining, peer lifecycle, LAN discovery and the server browser. Decisions below | |
| 12.2a/b | ✅ The AI rule engine — five real difficulties. Decisions below. ⚠️ **OPEN: no rule researches anything** (card `9.x-ai-research`) | |
| 12.3 | ➡️ **GROWN INTO PHASE 15.** Read Phase 15 instead of this row; what stays here is that PLAY already leads to the frame | |
| 12.4 | ✅ Save GAME. Decisions below. Replays remain a test fixture (0.7) — nothing player-facing plays one | |

⛔ **RECONNECT IS OUT OF SCOPE BY THE OWNER'S RULING**, so *nothing dials back in* is the FINISHED
behaviour and not the hole it reads as: *"we will not support reconnect after grace. in tunnel test
is fine, full reconnect after flight mode or app crash is not in scope"*. **The line is not how
long the silence lasted, it is whether the SOCKET survived it** — packet loss survives via
`LINK_TIMEOUT_*`, a dead socket ends that player's match. `DISCONNECT_GRACE` is therefore a
**notice period, not a recovery window**, and the rejoin token was ruled out with the rest.
**Desync detection is retired** — one authoritative sim, nothing to diverge from. Lag compensation
is the parked input-delay decision at the end of §12.1.

**The AI rule engine.** Five `data/ai_<level>.json` rule sets, each a flat list of `{do, when}`
evaluated in order, first match wins, **with no step pointer: the AI's state is the world.**
Conditions rather than timeouts, costs read from the real defs, reservation so an expensive goal
can save up past a cheap one, and a deterministic hashed reaction delay as the difficulty knob.
**`Docs/AI_Player_difficulty.md` is the authority on what each tier does.**

⚠️ **THE TIER LIST'S PROMISES TOOK FOUR ROUNDS TO BECOME TRUE IN PLAY, AND THE FIRST THREE WERE
INDIVIDUALLY UNOBSERVABLE** — a bot that marched on the nearest enemy in every mode; then a clock
longer than the match it was in; then an army it could not afford; then a farm it had built and
could not work. Each looked like "the AI does nothing". ⚠️ **The first rule that researches
invalidates every row of BUGS.md's ladder table**, which was measured with neither side
researching.

**Save GAME, and its purpose is not what "save/load" sounds like:** *"Save Game - when a map is
interupted by IRL so you can pick it back up with your friends or vs AI later."* The case that
defines it is a **multiplayer** match resumed on another day, not a solo autosave.

- A load is `SimWorld.new()` → `setup(cfg)` → `MapGen.build()` → `SaveGame.apply()`: **rebuild
  everything the `MatchConfig` determines and overwrite only what mutated**, so the file carries no
  second copy of facts two peers must agree on.
- **Entities are copied field by field and never re-spawned from their def**, because several
  fields are derived at spawn and then rewritten in play.
- **Three things mutate without being hashed and all three are saved:** `SimWorld._next_id`,
  `SimMap`'s four arrays, and `SimPlayer.vision`. `colours.json` order being load-bearing (§4) is a
  save-file constraint, not only a replay one.
- ⛔ **THE MULTIPLAYER HALF WAS NEVER WORLD STATE; IT IS WHO SITS WHERE.**
  `Net._next_free_player_id()` hands out the lowest free id in join order, so a returning friend
  lands in somebody else's town with every id involved perfectly legal, and a peer can be handed a
  seat `AISystem` is also driving. `Net.reserve_seats()` takes the roster out of the file.
- ⚠️ **THE ACCEPTANCE BAR CANNOT BE "peers agree on `state_hash()` after a reload".** `SimHost` is
  host-only and `_handle_tick` is connected inside `SimHost.build()`, so **a joined device steps no
  world at all** — it draws entities from snapshots and terrain from the config. The real bar:
  restore, compare `state_hash()` against the saved world IN ONE PROCESS, then step both tick for
  tick. (Stepping one alone passes with the entity-id counter dropped entirely.)
- **Distinct from Save MAP (11.3) and the two must never be blurred:** a saved *map* is the terrain
  and start layout a match was STARTED with; a saved *game* is a half-built settlement with rubble
  in it. Terrain needs no transfer because **no system in `src/sim/systems/` writes a terrain
  tile** — a tree is an entity on walkable ground (2.3).
...
- ⛔ **12.1b's other half, RECONNECT, IS CLOSED AS OUT OF SCOPE — owner's ruling, 2026-09-21.**
  *"we will not support reconnect after grace. in tunnel test is fine, full reconnect after
  flight mode or app crash is not in scope."* A client that dials back in, a rejoin token, and
  admitting a peer to a running world are **not wanted and will not be built.**

  ⛳ **THE LINE THE RULING DRAWS IS NOT HOW LONG THE SILENCE LASTED — IT IS WHETHER THE SOCKET
  SURVIVED IT**, and the two are indistinguishable on screen, which is the expensive thing to
  know here. **Packet loss** (a tunnel, a lift, an access-point handover) survives, because
  `LINK_TIMEOUT_MIN_MS` holds the socket open through it. **A socket that dies** (flight mode,
  interface down, a suspended app, a crash) ends that player's match, after ~10 s of notice.
  `BUGS.md` carries a firewall recipe because **a wifi toggle is the second case and can
  therefore never demonstrate the first** — that confusion already cost one playtest.

  ⚠️ **TWO CONSEQUENCES THAT ARE EASY TO GET BACKWARDS LATER.**
  **(1) `LINK_TIMEOUT_*` was "the cheap half of reconnecting" and is now the WHOLE recovery
  mechanism**, with nothing behind it — so weakening it costs matches, where before it merely
  postponed a hole somebody else was going to fill.
  **(2) `DISCONNECT_GRACE` changed meaning, and that is the substantive change here.** It read
  as a recovery window: hold the seat, hope they come back. Nobody can come back through it, so
  what the ten seconds buy is `GameScene._on_grace_tick` printing *"Player N will be
  disconnected in Ns"* every whole second — without them a player vanishes and is resigned in
  the same instant, and the survivors' only clue is a defeat notice for somebody who was alive a
  frame ago. ✅ **That also makes it judgeable by playing for the FIRST time**: it went 10 → 30 →
  10 in one day and neither value could have felt like anything, because a number gating an
  unbuilt mechanism is identical at every value. It is now the length of a countdown on screen.
  ➡️ **The case for shortening it is the stronger one** — every second is a second of an
  undefended town bought for a message delivered in the first one or two — and with the link
  timeout in front of it the survivors wait for the **sum**, ~25 s, which is the only number a
  playtest is actually feeling.

  ✅ Still true and still load-bearing: the **old player id** (`Net.reserve_seats()`, 12.4,
  2026-09-20) names which ids a joiner may be given and in what order; the fuse is lit **only
  inside a match**; and only the **ready ack** would put one out. That last seam is now
  unreachable **by ruling** rather than by omission, and is kept rather than deleted because it
  is the only honest moment to call a player back if the ruling is ever reopened. ✅ Telling the
  other players closed in `7c1d5d5`.
...
✅ **The phase is built** — the adult dragon, MapGen's nest and mother, the claim and its wire
form, fire and the cooldown. **13.3 was dropped by the owner without a line of it being written.**
The nest is composed entirely from existing gaia props; the dragon is 0 A.D.'s own
`fauna/dragon.xml`.

➡️ **OPEN — THE RIGGING SERVICE'S TERMS, AND IT IS A LICENCE QUESTION RATHER THAN CODE.** The mesh
and texture are 0 A.D.'s and CC-BY-SA 3.0, and the baked frames are too — **but the rig and the
clips are not 0 A.D.**, and §2.3's attribution table is narrower than that truth. **Owner's call
before this ships.**

➡️ **OPEN — THERE IS NOWHERE IN THE CAMPAIGN TO FLY A DRAGON YOU OWN**, and it is invisible from
every row here. Scenario 4's win row is *own 1 dragon*, so **the mission ends on the tick the claim
completes** — you are handed a dragon and the victory screen in the same moment. Scenario 5's map
has no nest. The only place to fly one is a skirmish. A content question for 16.10 or a sixth
scenario, not a bug.

#### 13.2 The nest is the ONLY route to a dragon — the rules of the claim

⚠️ **THERE IS NO CASTLE ROUTE TO A DRAGON.** §9's roster said Castle and this phase said
guardian-and-claim, and the two contradicted each other for months while both looked true.
`trainable_at` is `[]` and `building.castle.trains` has dropped it.

| | |
|---|---|
| **One dragon per map**, and she is **gaia's** | placed by MapGen with the nest |
...
| **Baby scale: 20%** | obviously a juvenile, still findable at phone zoom |

⛔ **UNIQUENESS IS A MAPGEN PROPERTY — place one nest, place one mother.** A `UnitDef.limit` plus
`UnitLimitSystem` was built against the castle reading and then **deleted**, because dropping
castle training left it with no consumer at all: nothing trains a dragon, so a train-time cap can
never fire. `git log` has it if regicide (11.2) ever needs one. **Do not rebuild it.**

⛔ **THE HATCHLING IS GAIA'S UNTIL IT GROWS, AND THAT IS THE WHOLE TENSION.** Handing the baby over
at spawn ends the feature on the tick it starts: the claimant walks it out of reach, and then there
is nothing to hold, nothing for a rival to deny, and no reason for the nest to be destructible.

⛔ **IT IS A SPAWN AND A DESPAWN, NOT A CHANGE OF OWNER.** Nothing in this game changes who owns a
gaia unit and nothing needed to. Mutating `def_id` and `owner_id` would leave a 300 hp dragon that
cannot fly — hp, armour, speed, domain and pop cost are copied off the def by `spawn_unit` and by
nothing else.

⛔ **`claim_owner` IS NEVER CLEARED**, which is the record half of "one dragon per map": a second
guardian dying at the same nest claims nothing. ⚠️ **AND LIVE STATE CANNOT ANSWER *"was the claim
denied"*** — "spent, and nobody got it" (`claim_owner` set, `claim_ticks_left == -1`) is
byte-for-byte the state a claim leaves when it **matures**, so a lose row written that way fires on
the winning tick, racing the win row beside it. The question has to be re-framed to *"is a dragon
still reachable"*: grown dragons, plus nests either unclaimed or still growing for you.

#### 13.2a What placing a guardian cost, and what still binds

- **`guards_post` is a different thing from "lives somewhere".** Five animals were the first kind
  and every wildlife rule was written for them; three of those rules get a guardian wrong.
- ⛔ **`CombatSystem._reacquire` COULD WALK THE GUARDIAN OFF HER POST.** She never *chooses* a
  target outside it, but she is handed them — measured camping an AI mining site 11 tiles past her
  reach for 3,000 ticks. `guards_post` carries a **leash** and exempts her from
  `SETTLEMENT_RADIUS`, without which one forward tower moves the dragon off the nest for good and
  the claim is free.
- **The nest's placement is arithmetic and every number is derived** — clearance is her
  `aggro_radius` + `UNIT_RING_RADIUS` + slack; off the segment between every pair of starts; no
  further from the board's centre than from the nearest start; her reach is `claimed` so neither
  tree pass grows a reason to walk in there. **The archipelago gets no nest, and that is arithmetic
  rather than a hole** — a test pins the impossibility so changing a number surfaces as a decision.
  *Whether a naval map should get a nest islet is the owner's call.*
- ⚠️ **THE MAP CENTRE IS ON EVERY ROAD**, because starts sit on a ring and a two-player pair is
  diametrically opposite through it. No tuning of a guardian's reach fixes a villager walking past
  her; making the nest block movement makes it *worse*, since A\* then hugs the footprint. It is a
  placement rule, not a radius.
- **`PathService.rebuild()` takes the domains `MapGen._domains_spawned` actually placed.** Grids are
  built per domain lazily, so the first flight re-swept every grid mid-match — 22 ms on two players
  against a 15 ms budget. **Nothing in terrain says a flyer is coming.**
- **`take_damage` requires `attacker_owner` with NO DEFAULT.** A default would have compiled all
  four existing damage sources unchanged and left the one nobody updated crediting nobody —
  presenting as *a dragon you cannot claim if you use the wrong weapon*. ⚠️ **`NO_ATTACKER` IS -1,
  NOT 0**, because owner 0 is gaia and gaia is a real attacker everywhere else.
- ⛔ **A DEFEATED CLAIMANT WOULD UN-END A FINISHED MATCH.** `SimPlayer.defeated` is a one-way latch
  and `WinConditionSystem` counts what a player **owns**, so a dragon handed to somebody knocked out
  four minutes ago puts them back on the board. The claim is abandoned rather than held open.
- **`visuals.json`'s `scale`** rides the canvas transform `_draw_frame` sets up for the mirror.
  `draw_set_transform` builds `T(position) * R * S`, so **`draw_offset` is NOT scaled** while every
  local coordinate is. ⚠️ **`scale` MOVES THE SPRITE AND NOTHING MEASURED IN METRES** — the ring,
  the health dot and the placeholder box read `footprint_m`/`height_m`, which are **authored** at
  20% rather than derived, because deriving them would run the projection inversion on numbers it
  was never measured for.
- 📝 **The nest's 34 props are sorted by `x+y`**, not by world y: `Iso._project` puts screen y at
  `(x+y) * half.y` and `EntityView` splits behind-from-in-front at `at.y < 0`, so sorting by world y
  gets both the split and the depth order wrong and looks like a renderer bug.
...
Withdrawn by the owner after playing her: *"drop healing, live test felt good without it."*

⛔ **THE ARITHMETIC IS KEPT BECAUSE IT APPLIES TO ANY FUTURE REGENERATION ANYWHERE IN THIS GAME: 1
HP PER TICK IS 10 HP PER SECOND.** The proposal described that as "roughly two elite swordsmen"; it
is nearer **seven** — a 3.3× understatement, and exactly the error a rate expressed *per tick*
invites, because nobody multiplies by ten while reading. The effect would have been a **cliff
rather than a difficulty knob**: eight elite swordsmen kill her in 50 s today and would have needed
five minutes; six could never have done it at any army size. (Her 8/8 armour already floors most of
the game to `MIN_DAMAGE` — a militia, archer, swordsman or villager does **1**.)

📝 If it ever returns it owes three answers, and the third can break 13.1: **whose** dragon heals;
**only at the nest or at any nest**; and ⛔ **does the HATCHLING heal — if it does, denial by
killing it is dead**, and razing 1200 hp of nest becomes the only route, which is the opposite of
what that branch exists for. Recommendation: exclude `unit.dragon_baby` in the def, not by radius.
...
ground blast is easier to add lets do it"* — so `units.json` declares `target: "ground"`,
`range: 5`, `radius: 2`: no new target shape in the sim, no swept tiles to fold into
`state_hash()`. **The line is still the better weapon and this is the row it would come back to**;
a blast at a tile is indistinguishable from a spell, and this weapon is supposed to come out of her
face.

⛔ **THE NUMBERS HAVE MOVED THREE TIMES AND THE SHAPE HAS NEVER MOVED** — now **500 / 400 / 300** by
Chebyshev ring on a 1200-tick cooldown. Each change came from a playtest, and `units.json` carries
`_note_fire_breath` so a later report of *"the mother feels weak"* finds the numbers first.

📝 **DOUBLING BOTH IS WHAT "DOUBLE THE DAMAGE" MEANS.** Doubling `amount` alone gives 500/450/400 —
*more* than double at the edge, and a flatter blast than the owner's three nested squares.

📝 **500 AGAINST 600 HP IS A DESIGN EDGE**, pinned by a test. The blast kills everything in the game
outright except another dragon, which survives on 108 hp: a mother who one-shots a mother makes the
120 s cooldown the entire dragon fight.

📝 **THE HEADER OF A THING IS NOT AN AUTHORITY ON THE THING** — `units.json`'s general `ability`
section described a weapon that had not existed for five days. Live values now live only beside the
fields they describe. This project has paid for that twice (§11.9 is the other).

**`ability_aim` GOES ON THE WIRE INSIDE `if ability_cooldown > 0`**, and that placement is why it
costs nothing: §12.1f's rule is that a field carried by *some* entities splits the roster into
another wire shape, and `ability_cooldown` had already paid that split.

`GameView` watches the cooldown **rise** — an **edge**, not "is it at maximum", because a dropped
or coalesced snapshot would swallow the effect, and at a 120 s cooldown that is the only breath the
fight was going to get. **No fog clause and none wanted**: a dragon the client cannot see is absent
from `updated`. ⚠️ **The picture is drawn at the size the sim burns**, `radius * 2 + 1` read off the
def — a view choosing its own would tell the player something untrue about where a fight happened.

⛔ **THE BUILDING FIRE'S ONE TRAP IS THE FOUNDATION.** A building under construction starts at a few
hit points and climbs (5.2), so *"below a third"* is TRUE for nearly every foundation for most of
its build time — an opening in which every pegged-out house is ablaze. The rule is therefore
...
#### The first MEASURED instance of this ceiling, 2026-09-04 (13.2a)

Everything above was written from the code, by the person who wrote the code. This is the first
time the ceiling was hit by running the game, and **it is worth reading because of how it
presented**: not as a lost battle, but as **a bot that could not afford an age.**

`test_ai_playtest` put an EASY bot on a 96-tile forest map with a dragon nest at the middle. It
built its opening, worked its ore out, and picked the next nearest wood — 60 tiles away, across
the board. The route passed 4.6 tiles from a 600 hp guardian that deals 30 damage to a 30 hp
villager. **It walked eight villagers into her, one at a time, over 3,000 ticks**, finishing the
run on 3 food with four units left, having never saved the 500 an age costs.

Three of the bullets above, all at once and all invisible in the log:

- *"the last fifteen died" is not a fact any rule can read* — so the twelfth villager took the
  same route as the first;
- *it cannot judge whether an attack worked* — it was not attacking at all, which is worse: the
  danger was not even a target, it was ground;
- *it cannot defend a place* — and equally cannot avoid one.

⚠️ **THE LESSON FOR WHOEVER REBUILDS THIS.** The obvious first condition is *"what does the enemy
have"*. This case needed something narrower and cheaper that no rule can express either: **"is
there anything hostile between me and where I am sending this villager"** — a property of a
ROUTE, not of an entity list. `AISystem` picks the nearest node of a kind by straight-line
distance and hands the tile to `PathService`; nothing in between ever looks at what the path
crosses. A danger map keyed by tile would answer it, and it would also answer *"where is the
fighting"* from the bullet list.

**It was worked around in `MapGenerator` rather than fixed here, and deliberately** — the nest
now keeps its distance from the roads between starts and nothing is grown inside the guardian's
...
*15.1–15.10 are all built: the data and loader, the objective vocabulary, the launch path, both
screens, the briefing and tracker, unlock, the five scenarios, the previews and RESET PROGRESS.
The rules they fixed:*

**Data and loading**

- **A folder with no `campaign.json` is not a campaign, and the shadow check must ask that BEFORE
  it calls anything shadowed** — an empty leftover directory otherwise reports the repo's own
  campaign as shadowing it.
- **`ContentImage.load_texture()` is the one place that opens a PNG outside `res://`.** `load()`
  and `ResourceLoader` cannot open these at all; the route is `Image.load_from_file` +
  `ImageTexture.create_from_image`. Null is the only failure mode — shareable content with a
  malformed icon should cost a placeholder, not a crash.
- **A broken campaign is shown and DISABLED with its first complaint as the blurb, never hidden.**
  One that silently vanishes is indistinguishable from one the game failed to find, and the player
  has a folder on disk they can see.
- ⚠️ **A pinned seed is not a pinned map** — §11.7's second trap.

**Screens**

- ⛔ **A LIST OF DISCOVERED CONTENT GOES INSIDE A `ScrollContainer` FROM THE FIRST LINE.** A
  `VBoxContainer` overflows — it does not clip, scroll or compress past its children's minimums —
  and the lobby shipped with its bottom strip off the screen for exactly this **while every
  structural test passed**, because a node asked for its rect returns that rect whether or not the
  window contains it. One campaign fits; the ninth would not, and the failure only appears on the
  machine that has the content. Assert the parent is a `ScrollContainer` and assert no measurement.
- **A live object crosses a scene change on a static, not through `change_scene_to_file`**, and it
  is one-shot — taken and cleared in `_init()`, so a second visit with nothing parked shows the
  empty notice rather than silently reopening the last one. A fifth autoload was rejected: §6.1's
  table is exactly four.
- ⚠️ **`_why_not_playable()` DELIBERATELY DOES NOT CALL `build_config()`**, which is the authority
  *and* the function that generates the map — a 192×192 generate every time the player browsed a
  row. The panel uses a cheap predicate; `launch()` asks the real one and wins if they disagree.
- ⛔ **A CONTROL BELONGS ON THE SCREEN THAT SHOWS THE STATE IT CHANGES.** RESET PROGRESS was built
  on the campaign list and the owner moved it: *"the reset is per campaign, so on the progress tree
  page not the campaigns page."* A campaign list shows a name and a blurb, so a reset there is a
  button you take on trust; the scenario list is a **column of locks**, and that column *is* the
  progress. The alert can then **name** what it clears. `reset(folder)` rather than `reset_all()`,
  with an empty folder refused, so a reset with no target can never come to mean "everything" — and
  the handler must **redraw**, or clearing the file leaves every mission looking unlocked.
- **`ConfirmOverlay` is an in-scene full-rect `Control`, NOT a `ConfirmationDialog`.** Godot's is a
  `Window` — screen coordinates, engine-sized buttons — so it is the one thing in the UI that would
  not scale with `canvas_items` stretch, and it needs a tree to `popup_centered()` at all. CANCEL
  takes the focus, never confirm. ⚠️ Danger red `#E65C5C` is **nearly invisible** on this theme's
  dark-red plate and read as *greyed out* — the one thing a live control must never look like.

**The tracker**

- ⛔ **IT RENDERS AND COMPUTES NOTHING.** `objective_progress`/`objective_done` are
  `ObjectiveSystem`'s. A tracker that counted villagers itself is a second evaluator, and the tick
  it disagrees is a player watching "14 / 14" while the match refuses to end.
- ⚠️ **A ROW'S INDEX IS ITS PLACE IN THE FULL LIST**, not on the panel — the arrays include alerts
  and losses, so a tracker numbering its own rows shows the house count against the villager
  target: two real numbers, wrongly paired, on a panel that looks correct. **Win rows only**; a
  lose row's progress reads as an instruction.
- **An alert is LATCHED by the sim and the first snapshot is PRIMED**, or a client joining
  mid-scenario opens with a banner about something that fired before it arrived.
- ⛔ **A TARGET OF ZERO IS COUNTED UPWARD.** *"✓ Kill the mother dragon 0 / 0"* is the honest
  measurement against the honest target and reads as a bug. `AT_LEAST` and `EXACTLY` do not share a
  form: "a number to arrive at" is true of `== 1` and **false of `== 0`, because nothing counts up
  to zero**. So the rule keys on the TARGET, not the comparison. **What it trades away, so nobody
  restores it:** the numerator is no longer how many remain, and cannot be — a true "killed 3 of 4"
  needs the count the row STARTED at, and no such baseline exists in the sim or on the wire. It
  reads `objective_done`, **never `measured == 0`**: the latch is the sim's.
- ⚠️ **NEW ROCKER HAS NO CHECK MARK** — `has_char` is false for U+2713/2714/2717 and for the
  bullets, squares and stars. A missing glyph draws tofu rather than failing, and the first
  completed objective got a literal `*`. The tick is `draw_polyline` and cannot be broken by a font
  swap. `MOUSE_FILTER_IGNORE` is asserted over the whole subtree, because **`mouse_filter` does not
  inherit**.

**Content and tests**

- **AN AUTHORED FIGHT'S DIFFICULTY IS A PROPERTY OF THE FIGHT**, and no test can say so. Scenario
  4's army was cut twice the same day — 155, then 75, then **25 elite swordsmen and nothing else**.
  155 is what "an army" looks like; 25 is what killing one dragon takes. The answer was twice to cut
  the army rather than raise the cap. Same shape as §11.8's *an authored target's reachability is a
  property of the map*.
- ⚠️ **NEVER HARDCODE HOW MANY SCENARIOS THERE ARE.** That constant broke six tests the day content
  was added, and `_at_progress(2)` silently stopped meaning "all unlocked".
- ⛔ **EVERY PROGRESS FUNCTION TAKES A `path`, DEFAULTED**, so the suite never touches the
  developer's own save — and with a button that *writes* the file that is against destroying real
  state, not just flaky assertions. The screen reads and clears through the same override, because
  showing one file's locks and clearing another's is worse than either. **Taking a screenshot must
  not have a side effect**: `preview_campaign` photographs the alert and presses CANCEL.
- ⚠️ **THE TESTS NEVER TRUST `user://` FOR LOCK STATE.** A developer who has played the campaign
  gets a different lock pattern from a fresh checkout — a test that passes for one person and fails
  for the next.
- **Nothing covers a scene change and nothing can without a second process.** The previews press
  with the screen **out of the tree** and instantiate the next scene **from the constant the screen
  itself names**, so a wrong constant appears as the wrong screen. What they cover is the
  **handoffs**, which is where a live object crossing between screens gets dropped.
...
So a scenario pinned by seed **would silently play a different map** after any `MapGenerator`
change, or on an ARM phone rather than an x86 desktop.

✅ **THE TRAP IS RETIRED AND NO SCENARIO IS PINNED BY SEED ANY MORE (2026-09-01).** All five name
a **file**: `ScenarioDef.build_config()` reads `MapFile` and never calls the generator, and a
scenario with no saved map **refuses to launch** rather than falling back — a fallback is how an
unpinned scenario would ship looking exactly like a pinned one. The seed still rides along inside
`meta` as provenance, which is what that field is for. Kept here because the reasoning is the
whole argument for 2.4c existing, and because the next thing tempted to pin a place by seed will
...
##### ⛔ OPEN, FOUND 2026-09-20: `unit` AND `building` CANNOT TELL "NONE LEFT" FROM "NO SUCH THING"

`ObjectiveSystem`'s header names this as trap 3 — *"`== 0` is a comparison an unimplemented or
unmeasurable subject passes"* — and lists three defences for it. **Count the vocabulary and two
subjects have none:**

| subject | what stops a typo becoming an instant loss |
|---|---|
| `area` | `_in_area` answers **−1** for an unknown region; `MapData.has_area()` keeps it askable; `build_config()` refuses the row at launch; 16.6's panel warns while authoring |
| `resource` | `RESOURCE_KINDS` is pinned, for `stock.get("foood", 0) == 0`'s reason |
| `ticks` | `_read_clock` refuses `<=`, which is satisfied on tick 1 and never again |
| `named_unit` | `SimWorld.named_units` is **declared at world build**, so a dead hero still answers and an undeclared one answers −1 |
| **`unit` / `building`** | ⛔ **nothing.** `id` is free text and `_sum` reads `entry[bucket].get(id, 0)` |

**It cost a real playtest on 2026-09-20**: the owner authored *"protect Sir Roland"* as
`subject: "unit", id: "SirRoland"` and lost the scenario on tick 1, because a def nobody has ever
declared has a count of zero. ➡️ **The fix is `area`'s shape, in the same three places** — −1 from
`_count` when the id names no def (**ask `GameDataRegistry`, never the census**, or *"leave the
enemy no militia"* stops working), a refusal in `ScenarioDef.build_config()`, and a warning in
16.6's panel. ⚠️ **And the panel should notice the SHAPE of the mistake**: an `id` with no dot in
it is never a def id, and *"did you mean subject `named_unit`?"* is the sentence that would have
saved the playtest. `_read_clock` is the precedent — a refusal that names what the author meant.
Card `16.x-unknown-def-id`, ✅ **closed 2026-09-21 (`0189383`, `efbc242`)**: `_count` returns **−1**
for an unmeasurable subject rather than 0, and `ScenarioDef.build_config()` refuses a scenario
naming a def id no data file declares.
...
##### The fifth owner: `gaia` (2026-09-06), for the mother dragon

`self` · `enemy` · `ally` · a player index could not express *"kill the mother dragon"*, and
the obvious spelling for a unique unit — `subject: "named_unit"` — is **16.7's and is
refused at load**. What she actually is, to anything that can count today, is a
`unit.dragon` belonging to **owner 0**; and `owner` had no way to say owner 0, because a
player number below 1 is refused as the typo it is everywhere else in the game.

So `ObjectiveDef.Owner` gained `GAIA`, spelled `"gaia"`, **appended** for `resource`'s
reason two subsections up — `owner` travels as an int, and inserting would renumber
`ENEMY`/`ALLY`/`INDEX` and reinterpret every objective in flight. `owner: 0` **stays
refused** and is not a synonym: a spelling says what an index cannot.

Two things make it narrow rather than a hole in the trap below:

- **It is the literal `[0]` and does not go through `Diplomacy`.** `allied(0, 0)` is FALSE
  by that class's own rule (*gaia allies with nobody*), so resolving gaia through
  `_side_of` would file it under not-allied — the **enemy** bucket — and *leave the enemy
  nothing* would mean *shoot every deer* again by a new road. `test_objectives` asserts the
  difference with two dragons per player against gaia's one.
- **`age` and `resource` refuse it at load.** `player_for(0)` is null, so those two would
  measure 0 for gaia — and 0 is a value a comparison PASSES. *"Gaia's age `<= 1`"* would be
  true on tick 1 of every match, forever. That is the `== 0` trap below arriving through the
  **owner** axis instead of the subject one, so it is refused in the same place.
...
clause somebody could delete. **`Owner.GAIA` does not weaken this** — what the trap forbids
is gaia arriving in a set the author did not ask for, and `gaia` is gaia arriving because
they typed it. See the subsection above.
...
- **`owner`** ∈ `self` · `enemy` · `ally` · `gaia` · a player index. This is the axis the AI
  famously does **not** have (Phase 14) and a win condition may have freely: the rule runs on the
  server, which can see the whole world. Route `enemy`/`ally` through `Diplomacy` and pass the team
  table — the argument is **required with no default** for §4.13's reason. **`gaia` is owner 0 and
  deliberately does NOT go through `Diplomacy`** (2026-09-06, for scenario 4's mother dragon); see
  §11.8's fifth-owner subsection for why, and for the two subjects it is refused with.
...
game; it is never shipped to a player and never runs on a phone.

**Its purpose in the plan's order is the SECOND half of the owner's sequence:** Phase 15 shipped
**five** scenarios on generated-then-saved maps, then this tool re-authors those maps for a custom
...
✅ **THE TOOL IS FEATURE-COMPLETE; WHAT IS LEFT IS 16.9 AND 16.10.** It opens a map, paints terrain,
places anything in the roster, draws areas, wall runs and **plateaus**, undoes all of it, validates
on save, authors win/lose conditions, names and re-stats individual entities, and exports a
playable campaign. **MapMaker 475/0.**

⛔ **THE MOST USEFUL THING THE OWNER'S VERIFICATION PRODUCED WAS ITS ONE FAILURE.** An exported
campaign **lost on tick 1**, because the objective was authored as `subject: "unit"` with
`id: "SirRoland"` rather than `subject: "named_unit"`. That is an authoring slip **and** a real
hole: `ObjectiveSystem._sum` reads an unknown def id as **0**, which passes `== 0`. ➡️ **The tool
did what it was built to do; the language it writes in had one subject that could not tell "none
left" from "no such thing"** — and the fix landed in §11.8's vocabulary, not here.
...
*Reviewed against the code 2026-09-04, before starting. **Six of the things this phase assumed
turned out to be wrong or missing**, and they are folded in below rather than left to be
discovered: decision 2's copy list is wrong, decision 7 is new and load-bearing, decision 8 is new,
and the table has gained 16.0, 16.2a, 16.4a and 16.4b. The rows that were already right were left
alone.*
...
2. **THE FILE FORMAT IS THE CONTRACT, NOT THE CODE.** Two Godot projects cannot share a `res://`.
   ...
   which is `tools/isobake.local.toml`'s precedent for exactly this problem. **A junction or
   ...
   **`MapMaker/format/` is 10 verbatim copies, 2 declaration shims and a 2-file presentation
   list.** The copies are `sim_map.gd`, `map_data.gd`, `map_file.gd`, `iso.gd`, `building_def.gd`,
   `resource_def.gd`, `game_defs.gd`, `map_validator.gd` (16.4b), `objective_def.gd` (16.6) and
   `cliff_plan.gd` (§11.10). The shims are `sim_world.gd` (`SUBTILE`) and `map_generator.gd` (the
   `Type` enum) — files too big to copy, so **one declaration is pulled out of the game's source
   and compared**. The whole enum is checked, not just the one name, because `meta.type` is a saved
   int and inserting a type in the middle would silently turn every recorded Desert into a Forest.

   ⚠️ **`cliff_plan.gd` IS THE ONE COPY THAT DECIDES WHAT THE TOOL *WRITES* RATHER THAN HOW IT
   READS.** A plateau gesture becomes ~40 entities by its rules, so a tool one revision behind the
   game lays a cliff the game does not agree with — which is the same class as a drifted objective
   parser, one level up. **Re-copy it after every change to `game/src/sim/cliff_plan.gd`**; the
   guard refuses to save until you do.
...
7. **NEW 2026-09-04 — BUMPING `FORMAT_VERSION` IS ALLOWED, AND IT IS A CHECKLIST RATHER THAN A
   ONE-LINE CHANGE.** ✅ **The owner's ruling, 2026-09-04:** *"not a problem, only tested on my
   mobile and laptop easy to updated. feel free to bump as needed."* So 16.5's "new field therefore
   a bump" stands — what was wrong was the assumption that a bump costs nothing, and the four
   things it actually costs are recorded here so a bump is a decision rather than a surprise:

   - **`MapFile.load_map()` refuses a mismatch outright** (`if version != FORMAT_VERSION`). There
     is **no migration path and no tolerance** — an old file does not degrade, it fails to load.
   - **Five committed `map.json` files** are on version 1 (`scenarios/HowToPlay/scenario_1`
     through `_5`), and they are the whole shipped campaign. **They must be rewritten in the same
     change as the bump**, which `preview_author_maps --force` can do today and 16.10 will do
     properly.
   - **They are also inside the published `howtoplay` content pack** as of 2026-09-03, so the pack
     needs a version bump and a re-download. Cheap at two devices, which is the owner's point.
   - **Two constants share this name** and they version different things — `MapFile.FORMAT_VERSION`
     (on disk) and `MapData.FORMAT_VERSION` (the wire). A field that only a *file* carries bumps
     the first; anything that rides a snapshot to a joining client bumps the second. **Areas ride
     both**, because `MapGen.build_from()` gets the map from `MapData` on every client.

   **Prefer an optional field where it is free** — `from_dict`'s `d.get(field, default)` shape
   already gives that for every field it reads, so "absent means none" costs nothing and skips the
   checklist entirely. Bump when the *meaning* of an existing field changes, which is what the
   constant is documented to be for.
8. **NEW 2026-09-04: THE TOOL IS NOT ALLOWED TO BE THE ONLY THING THAT CAN JUDGE ITS OUTPUT.** Two
   consequences, both now rows. **A map is validated on save** (16.4b) — `MapValidator.problems()`
   is already `static` and already takes a `MapData`, so the tool can run the game's own
   connectivity, resource-proximity and overlap checks with no new logic; an authored map is
   *more* likely to fail them than a generated one, because the generator was written to satisfy
   them and a person is not. And **the tool gets the game's `TestCase` harness** (16.1), since the
   copies of decision 2 are pure functions and a second Godot project with 2,158 tests on one side
   of the fence and none on the other is where the format quietly diverges.
...
| 16.6 | ✅ **The Map Conditions editor** — owner-verified 2026-09-20: conditions authored on a real map, the `<=` refusal read on screen, and a deleted last row that stayed deleted through a save and reopen. Writes §11.8's vocabulary and invents nothing: the same `subject`/`owner`/`compare`/`value`/`output` records. **It validates that there is at least one WIN and does not ask for a lose** — owning nothing is defeat on every map whatever it declares (Phase 15 decision 5), so a map whose answer is "beat them" needs **no conditions at all**. A time limit is one more row with `subject: "ticks"` — ticks, not seconds, for `ages.json`'s reason.<br><br>⛔ **THE STORED RECORD IS THE AUTHOR'S WORDS, NOT `to_dict()`'s INTEGERS.** `to_dict()` is the **wire** form — every enum an int, so the sim never re-parses a `">="` — and a `scenario.json` is the opposite. So `MapDocument.objectives` holds raw dictionaries and `ObjectiveDef.from_dict` is used only to **validate**. Storing parsed defs and writing them back produces a file of integers the game's own loader cannot read, and the tool looks perfect throughout because it never re-parses its own output.<br><br>⛔ **CONDITIONS HAVE ONE HOME, RESOLVED AS A FALLBACK CHAIN.** A `scenario.json` beside the map **wins and is read and written**; a map without one parks its rows in its own sidecar until 16.8 promotes it. They never both apply, it resolves **once at open**, and **`save()` re-resolves from the TARGET** — so a Save As into `maps/` moves the conditions with the copy instead of writing them back into the original scenario. The first cut stored them in the sidecar full stop, which meant the panel announced *"no conditions"* about a shipped scenario holding two win rows, and a row added there would have landed where **nothing in `game/src` reads it**. `side.erase("objectives")` is load-bearing for the same reason: `_preserved_header()` carries forward every key `to_dict()` does not derive.<br><br>⚠️ **`ScenarioFile` REPLACES ONE KEY AND RE-READS THE FILE AT WRITE TIME.** The tool can sit open for an hour over content two agents commit to; only the objectives array is its business. It also **puts integral floats back** — JSON has one number type, so a plain round trip turns `"seed": 815101` into `815101.0` on every number in the file. 📝 The first save of a hand-written `scenario.json` **reformats it** (`JSON.stringify` has one layout); nothing is lost, and a test round-trips the real `scenario_1.json` with its 40-line `_note`.<br><br>⚠️ **TWO MODE TRAPS ARE WARNINGS.** `last_man_standing` **refuses a file that carries objectives**, so adding one to scenario 3 authors a mission that will not start; and a `scenario`-mode file with **no win row can never be won**, the one case where an empty list is not healthy. The panel also says **which file a Save will write**, because it looks identical either way and that is how the first fault went unnoticed |
| 16.7 | ✅ **Per-entity overrides and named units** — the expensive row, and the only one with real sim cost. Health, attack, speed and a NAME on an individual placed entity; `MapGen.build_from()` applies them and ⛔ **`state_hash()` folds them in** — **the OVERRIDE, not the result**, since `SiegeSystem` rewrites `speed` on every pack and what two clients can differ about is what the FILE said. Named units are what `subject: "named_unit"` needed, and **`ObjectiveDef._NOT_YET` is now empty**.<br><br>⛔ **THREE SENTINELS, NOT ONE, BECAUSE 0 IS A REAL ANSWER TWICE.** `hp` unsets at **0** (nothing alive has 0 max hp); `attack` and `speed` unset at **−1**, because **0 attack is a deliberately disarmed unit** and **0 speed is every deployed siege engine**. Collapsing them discards the two meaningful zeroes. `MapData.clean_overrides` is the single place that decides and the inspector's spin-box floors are read from it.<br><br>⚠️ **A UNIT'S ATTACK IS NOT COPIED AT SPAWN AND A BUILDING'S IS**, which is why one idea has two shapes: `CombatSystem._damage_of` reads `def.attack_damage` at the blow so a mid-match tech reaches units already standing, while `SimBuilding` already held a copy. `attack_base_of` is public and static so the two readers cannot drift.<br><br>✅ **Owner-verified 2026-09-20** — a hero authored at 900/60/150 and played. ⚠️ **The name is sim-only and off the wire**, so the HUD shows the def's display name: a field on two units and absent on four hundred splits every unit into two `SnapshotSystem` shape tables. Its own row if ever wanted | |
| 16.8 | ✅ **Scenario export** — a `campaign.json` / `scenario.json` pair and the three icon slots beside the map, so a campaign is authored in the tool rather than by hand-editing JSON. 15.1's schema gets its second consumer, and the schema is the one that must not move.<br><br>⛔ **THE MODE IS DERIVED AND THERE IS NO PICKER** — `ScenarioDef` refuses three of the four combinations, so a mode control's wrong setting would author a mission that **refuses to start** for no expressive gain. Conditions → `scenario`; none → `last_man_standing`; **rows with no win row are refused outright**, which 16.6 only warns about because a half-finished map is an ordinary state to work in and an export is where it stops being one.<br><br>⚠️ **`ScenarioDef._read_map` NO LONGER DEMANDS A `seed`.** It was written when the seed WAS the map; since 2026-09-01 `build_config()` never calls the generator, and a map authored here has no seed at all. Required now only when **no saved map sits beside the scenario** — a pinned file is a complete answer. Writing `"seed": 0` was the alternative and would have been provenance that lies.<br><br>✅ **Owner-verified 2026-09-20** by exporting a campaign and playing it. ⚠️ **Export re-points the document** — `doc.dir` becomes the scenario folder — so the Conditions panel then edits the file the game reads, and the map is **saved twice** on purpose or the rows live in two files | |
...
**Closed off this table since**, one line each, because a done row in a queue is a row somebody
reads as work: **4.8 garrison + 4.9** (2026-08-27 — it did *not* close the walls hole it was
billed as closing, the owner ruled walls out; what it did close was `garrison_cap`, declared on
all 31 buildings since 0.4 and read by nothing, and it gave buildings an attack). **4.10
abilities, 4.12 stances, 4.14 formations** (2026-08-29). **9.3 + 9.4 the tech tree**
(2026-08-29). **2.4c the map save format** (2026-09-01, confirmed in play 2026-09-02).
**Phase 15 scenarios and campaigns**, all nine rows (2026-09-03) — 12.3's one-line row was
hiding a phase. **0.3 `AssetPacks`** (2026-09-03, hardware-verified under three network
conditions, the last open Phase 0 item). **5.3 building upgrades** (2026-09-03), which closed
Phase 5 with it. **13.1 + 13.2 dragons** (2026-09-04/06). **How To Play scenario 4, the dragon
claim as content** (2026-09-06) — 15.8's last content gap but one, taken out of 16.10 on the
owner's instruction. Each names the phase item that holds its detail.

**And since, to 2026-09-21:** **2.4d Archipelago** and the transports it demanded. **5.7 more
buildings**, 23 of them. **11.x King of the Hill** — zone, score, win, standings panel and a
sound on a change of control. **Phase 16, the MapMaker**, every row through 16.8 plus the file
shortcuts, the last three closed by the owner authoring and playing their output. **12.4
save/load** (2026-09-20), replays excepted. **10 control groups**, **0.3a** resumable pack
downloads, **13.2/13.4** the dragon claim and the nest, **12.x-lan** discovery and the server
browser, and the **2026-09-20 playtest queue** (#109–#118). ⛔ **12.1b reconnect left this table
by RULING, not by work** — see its row below, which is the one closure here that shipped nothing
and is the one most likely to be misread later.

**Still open, in the order it makes sense to take it.** ⚠️ **Rows that have since closed are
struck through and kept**, because the estimate each one got wrong is the useful part:
...
| ~~2.4d Archipelago~~ | — | — | ✅ **Done.** New map type; the validator's connectivity claim changed rather than relaxed, and the transports it demanded shipped with it. §11.6. ⚠️ **It still separates allies, one island each** — by design, and the card is explicit that whether that is wanted is a question for the owner rather than a bug |
...
| ~~5.7 More buildings~~ | — | — | ✅ **Done** — 23 buildings, ~70 bakes. The art track paced it exactly as this row predicted |
| **Phase 16 MapMaker** | High — it turns scenarios into content | High; a second Godot project | ⚠️ **NOW UNBLOCKED ON BOTH COUNTS AND THEREFORE UP NEXT** — it waited on Phase 15 and on 2.4c, and both closed. **It starts at 16.0**, which is new and is game-code rather than tool work: the skirmish saved-map picker and the pause-menu Save Map button are both specified in the owner's README, neither is built, and 16.2 cannot be verified without the first of them. 16.2 is still the row that proves the format contract; 16.7 (per-entity overrides) is the only row with real sim cost. **A 2026-09-04 review against the code changed six things in this phase before a line was written** — see §16's decision 2 and 7.<br><br>✅ **16.0, 16.1, 16.2 and 16.4b are done as of 2026-09-04, and the owner has authored a map in it.** What that first authored map taught is the row worth reading before picking the next one: it **saved without a murmur and was unplayable**, so 16.2's placement was hardened and 16.4b was pulled forward to sit behind the Save button. **Next is 16.4a (File ▸ Open), then 16.3 (the palette)** — in that order, because 16.4a is what 16.10's re-authoring needs and because a palette that cannot reopen its own output is a one-shot tool.<br><br>✅ **THE TOOL IS FEATURE-COMPLETE AS OF 2026-09-21 AND ONLY CONTENT IS LEFT.** Every row through 16.8 is closed, the last three (16.6, 16.7, 16.8) by the owner authoring and playing their output rather than by a suite, and `16.x-shortcuts` closed the four file actions the menus had. **Remaining: 16.9 (`HOW-To.md`) and 16.10 (the content).** ⚠️ The order above was right for a reason that only showed up afterwards — see §16 — so it is left standing rather than deleted |
| ~~12.1b reconnect~~ | — | — | ⛔ **CLOSED 2026-09-21 AS OUT OF SCOPE, NOT AS BUILT** (owner: *"full reconnect after flight mode or app crash is not in scope"*). LAN **discovery** landed 2026-08-31 and closed the friction point (typing an IP); **link patience** covers a few seconds of packet loss; a dead socket ends that player's match by design. The three things this row still wanted — a client that re-dials, a rejoin token, admitting a peer to a running world — are **not wanted**. ⚠️ **The one estimate here that was badly wrong is worth keeping:** *"low-medium, and lower than it was"*, on the grounds that two of three prerequisites were already built. The prerequisites were never the cost — **the cost was the rejoin token**, which is identity on a wire that carries none, and it is the piece the ruling declined. §12.1 |
...
| A.5 | UI chrome | ✅ **REPLACED WHOLESALE 2026-08-30 and the itch.io packs are GONE.** This row said "from the itch.io dragon packs, largely in use", and their licence forbade redistributing the originals — so three directories under `game/assets/ui/` were gitignored and **a clean clone had no HUD at all**. [P8] delivered 130 project-owned pieces plus two OFL font families, `licence_audit.py` went from 129 problems to PASS, and `game/assets/ui/` is now committed in full |
...
**0 A.D. HAS NO CLIFF TILE SET, AND THAT IS STRUCTURAL RATHER THAN AN OVERSIGHT — so A.13 builds
its own.** Surveyed 2026-09-09 against an Age of Empires II reference. 0 A.D. renders a cliff as
steep **heightmap terrain** with one of its **78 cliff textures** painted on it; the
`geology/*cliff*` actors are decorative rock **curtains** you embed in that slope. They have no
flat top — the heightmap was the top — open hanging bottoms, and no matching ends. Baking three
`temp_cliffs_a` segments end to end through the composite adapter confirms they do not butt: the
four meshes are 10.99 / 9.20 / 11.18 / 6.59 m long with top heights 7.81 / 7.81 / 5.00 / 9.25 m.
**AoE2 needs pre-rendered cliff tiles precisely because it has no true 3D terrain**, and 0 A.D.
never needed them.

What that leaves is raw material rather than a set. `vis.cliff` is **already baked and staged**
(899 KB, the Mediterranean furrowed face) but it is a 11.5 × 25.6 × 27.8 m **spire** — a
formation, not an edge. The route the owner took is the third one: **a cliff tile is a top quad
carrying our terrain texture plus a face quad battered down to z = 0 carrying one of 0 A.D.'s
cliff textures** — the same trick 0 A.D. itself uses, texture on geometry, with geometry we
control. `composite` already places and orients parts, so what is actually owed to the pipeline is
an **extruded/battered edge primitive** in the terrain adapter, which today makes flat quads only.

**And the cliff's height pinned down a project constant nobody had written: 1 m of world height
is 19.60 px of screen Y** — `pixels_per_metre` 22.627417 × cos 30°, the camera's elevation. (A
horizontal metre is different again: 16 px of x and 8 of y.) So a **4.0 m cliff draws a 78 px
face**, and 2 m of height is one tile's 39 px. Worth having written down, because every height
argument on this project has so far been conducted in metres and settled in pixels.

⚠️ **Getting there caught TWO published knight figures being wrong, and the second is the §4
trap.** The owner specified the cliff two ways — *"about 2 tiles high"* and *"2× as high as a
knight"* — which read as contradictory: 2 tiles is 4 m, while twice `visuals.json`'s declared
`height_m 3.34` is 6.7 m, and twice `knight.toml`'s recorded `inspect` figure of "8.266 m" is 16.5.
**`knight.toml`'s is raw 0 A.D. units mislabelled as metres** (so 4.13 m, and that is a bounding
box including a raised weapon rather than the animal), and the placeholder capsule is not a
measurement of the art at all. Measured off the **staged atlas** instead — the knight trims to
43–90 px over 384 frames, of which ~33 px is ground-footprint spread — the vertical extent is
~2.1 m, and twice that is 4.2. **Both of the owner's readings land on ~4 m once the knight is
measured rather than quoted**, which is this section's recurring lesson arriving in a new place:
atlas frames are pixels; measure them.
...
**Four findings outlived the rows they closed, and are kept as rules rather than history:**

- ⛔ **`yaw_offset_deg = 180.0` MUST STAY, AND THE ATLASES WERE MIRRORED RATHER THAN ROTATED.**
  Index 0 is a fixed point of the sign flip, so the half-turn is only *half* the correction and the
  game side's request to remove it was wrong. **The agreed check was blind to the fault by
  construction** — columns 0 and 4 are exactly the two a mirror about N–S leaves alone, so it
  passed twice and 242 atlases were re-baked against a theory that could never have worked, because
  **no rotation undoes a reflection**. Scope was 171 atlases, not the 82 units: walls and gates are
  `stored = 8` too and passed `preview_walls` because the swap is invisible on symmetric art.
  ⚠️ **Nothing in `game/` compensates for a bake** — a `directions_reversed` flag was built and
  reverted twice, and that is the owner's standing rule.
- **`act_enter`/`act_garrison` are ONE concept.** `act_leave` is the spare and stays unreclaimed;
  `unit.transport_ship` is the obvious second consumer.
- ⚠️ **THIS DOCUMENT CLAIMED AN `AudioManager` EXISTED FOR MONTHS WHEN THERE WAS NO SUCH FILE**,
  and it survived because the only things referencing it were code comments citing this plan. An
  audio id with no stream is silence by contract, so the gap was inaudible as well as invisible.
- **Two pipeline fixes that are not re-derivable:** quadruped animation transfer needs a per-clip
  `location_scale` correction — the two rigs describe one skeleton ~31× apart, and bone *length* is
  **not** a usable metric because the importer fabricates near-zero ones; and buried geometry needs
  `render.ground_clip` as a **3D cut at `z = 0`** rather than a crop, since `z = 0` projects to a
  diamond spanning most of the frame. The second unlocked 0 A.D.'s better ore sculpts, authored up
  to 64% below the ground plane.
...
| ~~A scenario pinned by seed plays a different map after any generator change~~ | **Retired 2026-09-01** | ✅ **Closed by making it impossible rather than by tolerating it.** All five scenarios name a saved **file**; `ScenarioDef.build_config()` reads `MapFile` and never calls the generator, and one with no map refuses to launch instead of falling back. The general rule survives the row — **provenance is not reproduction**, which 11.3 and 12.1 both argue — so anything tempted to pin a *place* by seed reads §11.7 first |
...
| **A CAMPAIGN REACHES A PHONE ONLY THROUGH `AssetPacks`** | Medium | `user://` on Android is not `adb push`-able and the dev override is editor-only, so a pack is the **only** delivery route to a device — everything else is exercisable on Windows and headless, which is where the gap hides. Two escapes that need no redesign: Godot's external-data-dir setting for a debug build, or a bundled root seeded into `user://content/` on first run, which stays one config line because the loader is a root **list** |
...
| **A map-borne cost is invisible to the whole suite** | Medium | ⚠ **HAS MATERIALISED.** `test_tick_cost` builds a **synthetic** world, so nothing in 2,800 tests had ever loaded a hand-drawn map and asked what it cost — and correct cliffs arrived at 9.20 ms a tick on the owner's own map with every test green (§11.10). Mitigation is `probe_map_cost.tscn`, which times the real file against itself with one def stripped, and **reports a difference rather than a number** so workstation noise cannot swamp it. The general form: **a synthetic fixture cannot price authored content** |
...
**WHERE THE BUILD IS, at `v0.9.81` — the tenth beta.** Suites **game 2824/0** (230,803 assertions)
and **MapMaker 475/0**, Godot pinned at 4.7.1. Phase 16's tool is feature-complete; **a map with
plateaus is drawn in it and a cliff is rock a unit cannot walk into** (§11.10).

⚠️ **A `Done` CARD IS NOT NECESSARILY A BUILT CARD.** The board has one bucket for "off the queue"
and at least three different reasons land in it — shipped, *needed no code*, and **discarded by the
owner** (`12.x-resumed-fog`, *"we will not add fog calculations"*; `12.1b` reconnect, out of scope).
Reading either of those as built is the mistake this paragraph exists to prevent.

**THE QUEUE, in dependency order.** Every row is a card on
[projects.dragoon.co.za/projects/2](https://projects.dragoon.co.za/projects/2); what is here is the
**order and the reasoning**, not a mirror of card state.

**1. `16.10` — THE CONTENT THE TOOL EXISTS FOR.** Re-author the five How To Play maps, then **"The
Dragon Born"**, authored entirely in the tool. Scenarios **3 and 5 are byte-identical**, so one
good duel map covers two rows.

⚠️ **SCENARIO 4'S GROUND IS GENERATED, NOT AUTHORED, AND RE-AUTHORING IT HAS THREE OBLIGATIONS.**
It was finished by regenerating from the same seed rather than waiting for the palette, which
bought a playable mission six weeks early and left the look owed here. **Whoever re-authors it must
keep the nest, the mother, and the 25-swordsman garrison** — `test_campaigns` asserts all three,
and 25 is a *measured* number cut twice from 155. The briefing is also pinned to where
`_place_nest` put the nest (the player's own bank, which is why the "across the river" line went).

**2. `16.9` — `MapMaker/HOW-To.md`.** Last on purpose, and writable now.

**3. PHASE 14 — THE AI'S ENEMY-BLINDNESS.** A rebuild of 12.2b's condition vocabulary, not a bug
fix, and it wants a settled balance first. Its first *measured* consequence: a bot walked eight
villagers, one at a time, into a 600 hp guardian, because *"the last one died there"* is not a fact
any rule can read. It cost the bot the age it was saving for — §14's ceiling arriving as an economy
failure with no mention of a dragon in the log.

**Then, in no strongly forced order:**

- **`9.x-ai-research` — the AI researches nothing.** Every `ai_profile` has declared `techs: true`
  against nothing since 12.2b; `ResearchCommand` exists and no rule emits one. A *balance* hole
  too: a human who buys Blast Furnace fights an army that never will. ⚠️ **BUGS.md's AI ladder tick
  table survives only while neither side researches, and the first rule that does invalidates every
  row of it.**
- ⛔ **`wall-facings-reachable` — BLOCKED ON AN A/B/C RULING, and tagged `game-code`.** Walls draw
  ~4 of 8 baked facings because the footprint system is axis-aligned: **2 are reachable and
  distinct** (along tile axis X, along Y), **4 are 180° twins** a symmetric wall draws identically,
  and **2 are true diagonals no `Rect2i` can ask for**. ⚠️ **The art is not what is missing and a
  re-bake would change nothing** — 20 of 22 wall and gate atlases carry a genuine 8 stored
  directions, counted. ⚠️ **And re-baking walls is never a free move**: `WallPlan.FACING_FOR_AXIS`
  was derived by *measuring the staged pixels* and `test_wall_facing` re-measures them every run,
  so a re-bake fails it by design. ➡️ **§11.10's `CliffPlan` is the worked precedent** — it solves
  the same axis-aligned-rect problem for cliffs, with blockers and a terrain byte.
- **`cliff-terrain`** — the remaining half is whether `MapData` should carry an explicit edge
  concept. §11.10 answered the *collision* question without one.
...
- **`3.5` camera-follow** and **`8.6` chat transport** — both long-declared, neither started. Chat
  has a layout and no transport; §8.2b specifies it as a reliable RPC pair on `Net`, **not** a
  `Command`.
- **`9.6` the age re-skin** — art-paced.
- **Three match-event sounds** — `7.x-match-events`, dragon slain, dragon tamed.
- **`6.x-farm-work-anim`** — a villager working a farm looks identical to one standing idle on it.
  The art side's `villager-farm-anim` is in `Test`.
- **`v1-readme`** — both READMEs at V1, and **four live pointers to `PROGRESS.md`, deleted in
  September**. Not dead links: active false claims about where status lives.
- **Naval combat.** Ships float and path, and transports load, but **nothing has ever fought at
  sea**, so a loaded transport crosses unopposed. Archipelago is what demands it, and it is not
  what makes that map playable.
...
- ⚠️ **THE TWO-DEVICE HANDOFF HAS NEVER BEEN TESTED**, and 12.4's closure does not cover it. The
  owner's defining case was *"pick it back up with your friends"*; what was played was the solo
  loop. `Net.reserve_seats`, the lobby roll-call and the bot-as-player-1 refusal have **13 tests
  and no hardware.** Recorded so the first two-device session is read as the first one — a fault
  found there is untested ground, not a regression.
- ⚠️ **ADDING AN ID TO `visuals.json` IS A PACK CHANGE**, because `build_packs.py` resolves packs
  from what the seam can ask for rather than from a directory listing. `preview_art_pack` is what
  catches the gap, and it must be run against the current `art_base` before a build goes to a
  phone: `res://` beats a mounted pack, so **this workstation cannot fail the obvious test.**

📌 **A named hero's name is sim-only and off the wire**, so the HUD shows the def's display name
and not "Sir Roland". `SnapshotSystem` groups `updated` by sorted field names, so a field on two
units and absent on four hundred splits every unit into two shape tables — a real cost, and its own
row if it is ever wanted.

- ✅ **`16.x-unknown-def-id`** (`0189383`, `efbc242`) — the tick-1 loss above. The one with a real
  player-facing cost, and the one whose fix changed a shared vocabulary rather than a screen:
  `_count` returns **−1** for anything unmeasurable, because `== 0` is a comparison an
  unimplemented subject *passes*.
- ⛔ **`6.x-farm-work-anim` — DIAGNOSED, AND IT IS NOT THE ART.** The owner reported farmers on
  fields *"look idle"*, and they are: `AnimationSystem`'s GATHER branch casts the target to
  `SimResourceNode`, **a field is a `SimBuilding`** that `GatherSystem.is_harvestable()` answers
  for, the cast returns null, and because the chain is `if/elif/elif` the whole thing falls
  through to `return &"idle"`. The villager atlas has `work_build`/`work_chop`/`work_hunt`/
  `work_mine` and **no farming clip**, so the fix is choosing one for the building case. Whether
  a real farming clip is worth baking is asked in `asset_request.md`.
- ✅ **`8.x-action-column-overflow` — CLOSED WITH NO CODE CHANGE, AND THAT IS THE FINDING.** The
  diagnosis was right — `_capped()` slices the tail silently and the castle emitted nine with a
  rally point set — but **8.9 had already removed the overflow** by moving the detail grid onto
  its own page. The instruction to *"confirm in a match before fixing"* is what caught it.
  ⚠️ **What remains is ZERO HEADROOM**: the worst case measured is exactly `MAX_ACTIONS`, so the
  next action added to any building re-opens this silently. **The owner ruled: no follow-up
  card**, which is a decision and not an oversight.
- ✅ **`8.x-build-menu-modal`** (§8.9, `085f83c`, confirmed in play) and ✅ **`8.x-destroy-confirm`**
  (§8.10, `5450eff`) — both designed in Phase 8 rather than here, and both built as designed.
- ✅ **`8.x-chat-align`** (`c47f5b0`) — the owner's ruling was *every* message, and **the target
  was not the CHAT page this queue assumed** (§8.6): it is the in-match `MessageLog` above the
  minimap, the block that prints the disconnect notices. One line.
- ✅ **`8.x-train-on-foundation`** (`c37011d`) and ✅ **`4.x-build-stations`** (`e8fe4ed`) — a
  building still going up offered four train buttons that did nothing, and twelve builders sent
  to a castle left three standing idle. Filed after this queue was written, hence #117/#118.
- ✅ **`16.x-shortcuts`** (`4bad8ac`) — `Ctrl+S`/`Shift+S`/`O`/`N` in the MapMaker. ⚠️ The binding
  went on `_input`, not `_shortcut_input` and not `Button.shortcut`: 16.2a paid for that with
  `Ctrl+Z` and blamed the canvas, when being *after the GUI pass* was the fault. **And only
  `Ctrl+Z`/`Y` defer to a focused text field** — `defers_to_text_field()` is that rule, made a
  pure static so it is assertable without a keyboard. ⛔ **File ▸ New and File ▸ Open still have
  no unsaved-changes guard, and the owner ruled 2026-09-21 to leave it** (*"leave it as is"*).
- ⛔ **`12.x-resumed-fog` — DISCARDED BY THE OWNER, 2026-09-21, NOT IMPLEMENTED.** *"we will not
  add fog calculations - disgard 115."* A resumed match re-exploring from scratch is now the
  **intended behaviour** rather than a known gap; the owner had already said so closing 12.4
  (*"fog of war resets but thats acceptable"*) and said it again harder. The seed-at-match-start
  design on the card is **not to be built**, and `ClientFog` stays as 12.1f made it.
  ⚠️ **AND THE LOOSE THREAD THAT MUST NOT BE TIDIED:** `SaveGame` still stores and restores
  `SimPlayer.vision`, and it must keep doing so — `SnapshotSystem._entry_for` reads the server's
  vision to decide what to send each player, so that grid is **what the host transmits**, not the
  picture on screen. Reading this closure as licence to drop it would be treating a decision
  about *drawing* fog as a decision about *computing* it.
- ⛔ **`6.x-farm-work-anim` IS THE ONE STILL OPEN**, and it is the row below rather than a
  forgotten one: the sim half is game-code and the clip is a question in `asset_request.md`.
...
a mirror about N–S leaves alone.** It passed twice while every unit in the game was reflected. Any
facing check reads columns **2 and 6** as well. §13.2's closed-row list keeps the four lessons.
