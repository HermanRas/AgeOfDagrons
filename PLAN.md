...
| A.13 | **Cliffs as an AoE2-style tile set** — straight, outer corner, inner corner, forming a connected edge with a walkable-looking top. Owner's call 2026-09-09 after the survey below | 🔜 **To-Do, board card `cliff-tiles`, and BLOCKED ON FOUR ANSWERS — two of which are the game side's.** (1) cliff **height in metres**, which decides whether a unit behind one is occluded; (2) whether a cliff is **impassable sim terrain or pure decoration**, which decides whether it needs a footprint and a collision story at all; (3) **how a map marks a cliff edge** — `MapData` has no edge concept, only `{def_id, player, tile, size_class}`; (4) **tile length** — one 2 m tile per piece or longer runs — which is mine to propose once the first three land, because occlusion height and map encoding constrain it. **Nothing is prototyped**: the survey below is measured, the build plan is not |
