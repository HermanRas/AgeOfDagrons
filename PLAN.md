...
| # | Item | Tag |
|---|---|---|
| 5.1 | ⚠️ **PARTIAL** -- `PlaceBuildingCommand` pays the cost and claims a FOUNDATION-phase footprint, validated the same way every other command is: `can_afford()` plus `SimMap.can_place_building()`, checked once in `validate()` and trusted in `apply()` since nothing else can run between them for the same command (CommandSystem is sequential). Snap-to-grid, validity colour and a red flash on invalid are all there, but **not as a drag** -- `PlacementGhost` previews and commits in a single tap instead. `GameScene` reads `Net.host().world` to colour the ghost, the same documented solo-only exception `_start_match()` already uses. Verified on device: entering build mode, an unaffordable placement correctly spends nothing and stays in build mode (starting stock is 0 -- nothing has been gathered yet). **Planned resolution, decided but not yet built**: one-finger pan stays the default everywhere outside build mode -- ruled out swapping it to two fingers, since that is also box-select's own trigger (8.3) and one-handed play needs pan on a single thumb. Instead, entering build mode LOCKS `CameraRig` (no pan, no edge-swipe zoom) and hands its one-finger drag to `PlacementGhost` instead -- the ghost follows the finger, snapped to the grid, coloured live by validity; release commits it if valid, otherwise the ghost simply reverts (no separate cancel gesture needed for a bad drop). Placing or hitting Cancel Build unlocks the camera again. This touches `CameraRig` (a lock flag it already has no equivalent of) and `InputRouter` (routing its drag to the ghost instead of the camera while locked), neither of which 5.1 has changed yet | **[MVP]** |
...
| 5.5 | Destruction Ã¢â€ â€™ rubble, frees tiles, unselectable | **[MVP]** |
