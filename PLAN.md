...
10. ✅ **Tap targets for tall and small art** (4.3, 2026-08-23), both reported off screenshots. Picking still goes by the tile under the finger — but the *art* is not drawn on that tile, and that was the whole bug. **When the tile holds nothing**, the tap now falls back to any resource node whose art is painted over it, asked as `Occlusion.hides()` — literally the same call, with the same measured `{rect, pad, reach}`, that decides whether a villager behind a tree gets outlined. *If the art hides that tile, tapping it is tapping the art*, and those two answers have to agree. Three properties come free: tiles **in front** are never covered (so walking past a tree still works), the **screen column** rejects taps that are behind in depth but visibly beside, and the reach is **measured** off `height_m` rather than tuned per kind. It took two goes — the first tried a 2x2 `pick_footprints` on the tree, which reaches 48 px against 157 px of trunk, and no tile rect can grow up-screen without also eating the ground in front. `res.tree`'s footprint never moved: one tile, forest still walkable. `PICK_COVER_REACH_TILES` is the dial if forests turn sticky.
