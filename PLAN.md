...
- **12.1b's other half, RECONNECT, is still open — and is now down to ONE thing.**
  ...
  told the other players the match is holding its breath — ✅ **closed in `7c1d5d5`**, in the
  panel chat will inherit; until then a silent ten seconds read as
...
`16.x-unknown-def-id`, and §11.8's vocabulary is where the fix lands, not here. ➡️ **The tool did
...
4.7.1. ✅ **The `Test` column is EMPTY** — 16.6, 16.7, 16.8 and 12.4 were all closed by the owner
playing them on 2026-09-20, and **eight cards (#109–#116) were filed out of that one session**. The How To Play pack is published at v4 and the **art packs are live** — `art_base_v2`
...
two subjects nobody defended. Card `16.x-unknown-def-id`; the fix is `area`'s shape applied to
`unit`/`building` (−1 from `_count`, a refusal in `build_config()`, a warning in the panel).
...
- **12.1b reconnect — down to ONE thing, the client half.** Discovery landed 2026-08-31 and
  ...
  lands. ✅ **The "nobody is told" half closed in `7c1d5d5`** — a match holding its breath now
  says so, in the panel chat will inherit. A silent ten seconds used to read as a crashed host.
...
**THE 2026-09-20 PLAYTEST QUEUE (#109–#116).** Eight cards out of one session, listed here for
the *shape* of each; the cards carry the detail.

- ⛔ **`16.x-unknown-def-id`** — the tick-1 loss above. The one with a real player-facing cost.
...
- ⛔ **`8.x-action-column-overflow`** — `_capped()` slices the action column silently and the
  castle already emits nine with a rally point set, so **Destroy is being dropped today**.
  Derived from two comments that are each right; **confirm in a match before fixing**.
- **`8.x-build-menu-modal`** (§8.9) and **`8.x-destroy-confirm`** (§8.10) — both designed, both
  written up in Phase 8 rather than here.
- **`8.x-chat-align`** — needs the owner's ruling on whose messages (§8.6).
- **`16.x-shortcuts`** — `Ctrl+S`/`Shift+S`/`O`/`N` in the MapMaker. ⚠️ The binding goes on
  `_input`, not `_shortcut_input` and not `Button.shortcut`: 16.2a paid for that with `Ctrl+Z`
  and blamed the canvas, when being *after the GUI pass* was the fault.
- **`12.x-resumed-fog`** — a resumed match redraws fog from scratch. **The save is not what lost
  it**: `SimPlayer.vision` is stored and restored faithfully, and 12.1f took fog off the wire, so
  `ClientFog` accumulates from tick 1 of a fresh session. Owner called it acceptable. The fix
  neither option in `ClientFog`'s header covers is to **seed the client once at match start** —
  the ~9 KB 12.1f removed ten times a second, paid once, per-player and filtered.
