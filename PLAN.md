...
| # | Item | Tag |
|---|---|---|
| 5.1 | ⚠️ **PARTIAL** -- `PlaceBuildingCommand` pays the cost and claims a FOUNDATION-phase footprint, validated the same way every other command is: `can_afford()` plus `SimMap.can_place_building()`, checked once in `validate()` and trusted in `apply()` since nothing else can run between them for the same command (CommandSystem is sequential). Snap-to-grid, validity colour and a red flash on invalid are all there, but **not as a drag** -- `PlacementGhost` previews and commits in a single tap instead. `CameraRig` already claims a one-finger drag for panning (8.3), and giving placement its own continuous position feed would mean the two gestures fighting over the same finger; a proper live-drag ghost needs that resolved first, which is real work belonging to its own pass rather than something to rush alongside 5.4. `GameScene` reads `Net.host().world` to colour the ghost, the same documented solo-only exception `_start_match()` already uses. Verified on device: entering build mode, an unaffordable placement correctly spends nothing and stays in build mode (starting stock is 0 -- nothing has been gathered yet) | **[MVP]** |
